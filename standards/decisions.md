...
## D-026 — Три входа в базу поверх одного ядра (2026-08-07)

**Контекст.** Единственным способом спросить базу был Claude Code со скиллом. Замер
показал, где на самом деле время: сам поиск — 0,11 с, а ход в диалоге — 4m19s.
Разница не в базе, а в модели: сессия накопила 12,7 МБ переписки, и каждый ход
перечитывает её целиком. Ускорять поиск дальше бессмысленно — надо было убрать
модель с пути, когда она не нужна, и дать доступ оттуда, где Claude Code нет
вовсе (телефон).

**Решение.** Один поисковый движок, три входа:

- `scripts/kbapi.py` — структурный ответ (словарь) поверх `ask.py`; ранжирование,
  пороги уверенности и правила триажа остаются в одном месте
- `bin/kubepedia` — терминал из любого каталога
- `scripts/serve.py` + `webui.html` — резидентный процесс, веб-чат, HTTP-ручка
- `scripts/bot.py` — телеграм, доступ с телефона

**Почему так, а не иначе.**

*Ядро отдельно, печать отдельно.* `ask.py` совмещал разбор и вывод. Три входа,
каждый со своей копией ранжирования, разошлись бы в ответах — и разошлись бы молча.
`kbapi` возвращает данные; терминальная печать `ask.py` не изменилась ни на символ.

*Резидентность вместо кэша.* CLI платит чтение кэша на каждый запуск, сервер не
платит ничего: корпус в памяти, ответ 25 мс. Правки базы процесс замечает сам по
подписи файлов — иначе долгоживущий процесс молча отвечал бы из копии, снятой на
старте, и никто бы не догадался его перезапустить.

*Длинные опросы, а не вебхук.* Бот ходит наружу сам: не нужны белый адрес, проброс
портов и сертификат, работает из-за NAT. Плата — пока машина спит, бот молчит;
это ограничение размещения, а не устройства.

*Доступ к боту закрыт списком chat_id.* Бота находят по имени, и без списка
внутренняя инженерная база отвечала бы случайному человеку. Незнакомому бот
называет его chat_id и молчит по существу.

*Модели в цепочке нет.* Ответ приходит мгновенно и одинаков каждый раз, но и
пересказа живым русским нет — показывается то, что записано в базе. Это осознанный
размен: он и был целью. Скилл в Claude Code остаётся для случаев, где нужен именно
разбор с рассуждением.
