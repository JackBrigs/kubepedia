...
## Project Version
The current Kubepedia version is `0.6.0`.
Version `0.2.0` established the architecture, execution model, source policy, knowledge format, and validation rules; `0.3.0` was the first large **content** release on that architecture (Kubespray range v2.27.0–v2.31.0 / K8s 1.29–1.35, the application-platform addon catalog and its deep docs, the latest-upstream upgrade horizon, and the expanded troubleshooting layer — `standards/decisions.md` D-016). `0.4.0` added the **usability & assurance** layers on the same architecture: the operational **runbook** layer (15 canonical operations, `CONCEPT-RUNBOOKS_INDEX`), the **security posture** layer (`CONCEPT-SECURITY_INDEX`, `CONCEPT-INSECURE_DEFAULTS`, CVE matrices), the **retrieval/AI-first** layer (tag/alias facet indexes, `CONCEPT-KB_NAVIGATION`, D-018), the KB-driven **Upgrade & Change Report** capstone, and the **automated consistency guard** (per-tag version-drift check + validator guards — D-019). `0.5.0` is the **upstream-mined depth** release on the same architecture: version-accurate mining of the actual upstream repos at the exact tags Kubespray ships — Cilium/Envoy and Argo CD **version-jump upgrade** docs, the Kubernetes **KEP** layer (silent default-flips + curated Urgent Upgrade Notes for 1.29–1.35), and **core-component/Talos depth** (external cloud-controller-manager, custom_cni/ovn4nfv, CoreDNS customization, Talos↔K8s matrix / talosctl cluster / Cluster API / Omni). See `standards/decisions.md` D-020. `0.6.0` is the **consumer-handles & operational depth** release: a single `kubepedia` CLI over every tool (symptom search with raw-output triage, inventory migration diff, personalised upgrade report, post-upgrade verification, graph impact, per-tag version lookup, CI gate, freshness monitor), periodic guards (osv.dev CVE re-sweep — D-021/D-022 — freshness and per-tag version drift), a self-improvement loop (`learn.py`, `bench_search.py`), retrieval rewritten to BM25/IDF, the completed addon layer and blind-spot sweeps, ITIL 4 service-management templates, and two incident-born layers absent upstream — **tuned** and **Ubuntu networking** — plus the AWX job-template layer over the runbooks. See `standards/decisions.md` D-023.
