...
- **Контроллер Gateway API перезапускается по расписанию, а под всё время `Running`** — срывается
  продление аренды лидерства, контейнер добивает проба живости → [[TROUBLE-ENVOY_GATEWAY_RESTART_LOOP]].
