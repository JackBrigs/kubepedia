...
### Что уже лежит в `master` за потолком v2.31.0 (проверено 2026-07-28)

Отдельная от «горизонта апстрима» строка наблюдения: горизонт показывает, что выпустили сами
проекты, а это — что из выпущенного **уже подобрал Kubespray** и, значит, приедет в следующем
релизе. Источник: `roles/kubespray_defaults/defaults/main/download.yml` и
`vars/main/checksums.yml` на `master`, влито батчами PR #13376 и #13393 (2026-07-24…25).

| Компонент | v2.31.0 | `master` | Чем это важно |
|---|---|---|---|
| containerd | 2.2.3 | **2.3.3** (первый ключ чек-сумм) | 2.2.3 несёт 6 CVE, фиксы в 2.2.5 / 2.3.2 — на `master` они уже есть |
| Calico | 3.31.5 | **3.31.6** | ровно та версия, где закрыты `CVE-2026-41184` и `CVE-2026-41185` |
| runc | 1.4.2 | **1.4.3** | закрывает `CVE-2026-41579` |
| Cilium | 1.19.3 | **1.19.6** | закрывает `CVE-2026-53935` — последнюю открытую в матрице Cilium |
| Kubernetes | 1.35.4 | чек-суммы **1.36.2 / 1.36.3**, 1.35.7, 1.34.10 | 1.36 перестаёт быть недостижимым |

**Вывод, который стоит держать в голове при планировании:** следующий релиз Kubespray закрывает
**все** открытые CVE из матриц по этим четырём компонентам ([[CONCEPT-SECURITY_INDEX]]). Сегодня
их приходится закрывать пином и ручной чек-суммой; после релиза — обычным апгрейдом.

**Практическое следствие уже сейчас.** Чек-суммы для containerd 2.2.5 и выше на `master` уже
посчитаны, поэтому пин `containerd_version: 2.2.5` на v2.31.0 больше не требует считать sha256
самому — значение берётся из `vars/main/checksums.yml` ветки `master`
([[TROUBLE-CONTAINERD_KNOWN_CVES]]).

Ничего из перечисленного **не выпущено**: v2.31.0 датирован 2026-04-24, изменения влиты в июле.
Планировать по ним можно, ссылаться как на поведение релиза — нет.
