---
id: VARIABLE-EXTERNAL_ORACLE_COMPARTMENT
type: variable
title: external_oracle_compartment
status: active
...
Kubespray variable `external_oracle_compartment` — default `(empty)`. Defined in `roles/kubernetes-apps/external_cloud_controller/oci/defaults/main.yml`. Present in Kubespray
...
```yaml
external_oracle_compartment: (empty)
```
