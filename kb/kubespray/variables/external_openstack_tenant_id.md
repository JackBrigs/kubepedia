---
id: VARIABLE-EXTERNAL_OPENSTACK_TENANT_ID
type: variable
title: external_openstack_tenant_id
status: active
...
Kubespray variable `external_openstack_tenant_id` — default `{{ lookup('env', 'OS_TENANT_ID') | default(lookup('env', 'OS_PROJEC…`. Defined in `roles/kubernetes-apps/external_cloud_controller/openstack/defaults/main.yml`. Present in Kubespray
...
```yaml
external_openstack_tenant_id: {{ lookup('env', 'OS_TENANT_ID') | default(lookup('env', 'OS_PROJEC…
```
