---
id: VARIABLE-INGRESS_NGINX_SERVICE_NODEPORT_HTTP
type: variable
title: ingress_nginx_service_nodeport_http
status: active
...
Kubespray variable `ingress_nginx_service_nodeport_http` — default `(empty)`. Defined in `roles/kubernetes-apps/ingress_controller/ingress_nginx/defaults/main.yml`. Present in Kubespray
...
```yaml
ingress_nginx_service_nodeport_http: (empty)
```
