---
id: VARIABLE-GCP_PD_RESTRICTED_ZONES
type: variable
title: gcp_pd_restricted_zones
status: active
...
Kubespray variable `gcp_pd_restricted_zones` — default `(structured / block value — see source)`. Defined in `roles/kubernetes-apps/persistent_volumes/gcp-pd-csi/defaults/main.yml`. Present in Kubespray
...
```yaml
gcp_pd_restricted_zones: (structured / block value — see source)
```
