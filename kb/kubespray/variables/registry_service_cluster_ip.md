---
id: VARIABLE-REGISTRY_SERVICE_CLUSTER_IP
type: variable
title: registry_service_cluster_ip
status: active
...
Kubespray variable `registry_service_cluster_ip` — default `(empty)`. Defined in `roles/kubernetes-apps/registry/defaults/main.yml`. Present in Kubespray
...
```yaml
registry_service_cluster_ip: (empty)
```
