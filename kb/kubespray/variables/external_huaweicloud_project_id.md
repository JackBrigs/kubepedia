---
id: VARIABLE-EXTERNAL_HUAWEICLOUD_PROJECT_ID
type: variable
title: external_huaweicloud_project_id
status: active
...
confidence: verified
aliases:
  - external_huaweicloud_project_id
tags:
  - kubernetes-apps
...
Kubespray variable `external_huaweicloud_project_id` — default `{{ lookup('env', 'OS_TENANT_ID') | default(lookup('env', 'OS_PROJEC…`. Defined in `roles/kubernetes-apps/external_cloud_controller/huaweicloud/defaults/main.yml`. Present in Kubespray
...
```yaml
external_huaweicloud_project_id: {{ lookup('env', 'OS_TENANT_ID') | default(lookup('env', 'OS_PROJEC…
```
