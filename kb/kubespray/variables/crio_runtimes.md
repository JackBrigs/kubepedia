...
Kubespray variable `crio_runtimes` — default `(structured / block value — see source)`. Defined in `roles/container-engine/cri-o/defaults/main.yml`. Present in Kubespray
...
```yaml
crio_runtimes: (structured / block value — see source)
```
