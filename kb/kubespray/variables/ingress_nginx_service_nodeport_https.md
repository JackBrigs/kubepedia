---
id: VARIABLE-INGRESS_NGINX_SERVICE_NODEPORT_HTTPS
type: variable
title: ingress_nginx_service_nodeport_https
status: active
...
Kubespray variable `ingress_nginx_service_nodeport_https` — default `(empty)`. Defined in `roles/kubernetes-apps/ingress_controller/ingress_nginx/defaults/main.yml`. Present in Kubespray
...
```yaml
ingress_nginx_service_nodeport_https: (empty)
```
