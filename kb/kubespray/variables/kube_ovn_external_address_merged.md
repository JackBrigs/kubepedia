---
id: VARIABLE-KUBE_OVN_EXTERNAL_ADDRESS_MERGED
type: variable
title: kube_ovn_external_address_merged
status: active
...
Kubespray variable `kube_ovn_external_address_merged` — default `(structured / block value — see source)`. Defined in `roles/network_plugin/kube-ovn/defaults/main.yml`. Present in Kubespray
...
```yaml
kube_ovn_external_address_merged: (structured / block value — see source)
```
