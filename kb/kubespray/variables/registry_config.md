---
id: VARIABLE-REGISTRY_CONFIG
type: variable
title: registry_config
status: active
...
Kubespray variable `registry_config` — default `(structured / block value — see source)`. Defined in `roles/kubernetes-apps/registry/defaults/main.yml`. Present in Kubespray
...
```yaml
registry_config: (structured / block value — see source)
```
