---
id: VARIABLE-EXTERNAL_ORACLE_AUTH_KEY
type: variable
title: external_oracle_auth_key
status: active
...
Kubespray variable `external_oracle_auth_key` — default `(empty)`. Defined in `roles/kubernetes-apps/external_cloud_controller/oci/defaults/main.yml`. Present in Kubespray
...
```yaml
external_oracle_auth_key: (empty)
```
