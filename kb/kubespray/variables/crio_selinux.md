...
sources:
  - type: code
    ...
    note: "default: {{ (preinstall_selinux_state == 'enforcing') | lower }}"
relations: []
---
...
Kubespray variable `crio_selinux` — default `{{ (preinstall_selinux_state == 'enforcing') | lower }}`. Defined in `roles/container-engine/cri-o/defaults/main.yml`. Present in Kubespray
