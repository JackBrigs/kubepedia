---
id: VARIABLE-SCHEDULER_PLUGINS_DISABLED_PLUGINS
type: variable
title: scheduler_plugins_disabled_plugins
status: active
...
Kubespray variable `scheduler_plugins_disabled_plugins` — default `(structured / block value — see source)`. Defined in `roles/kubernetes-apps/scheduler_plugins/defaults/main.yml`. Present in Kubespray
...
```yaml
scheduler_plugins_disabled_plugins: (structured / block value — see source)
```
