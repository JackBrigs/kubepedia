---
id: VARIABLE-KUBE_OVN_CENTRAL_IPS
type: variable
title: kube_ovn_central_ips
status: active
...
Kubespray variable `kube_ovn_central_ips` — default `(structured / block value — see source)`. Defined in `roles/network_plugin/kube-ovn/defaults/main.yml`. Present in Kubespray
...
```yaml
kube_ovn_central_ips: (structured / block value — see source)
```
