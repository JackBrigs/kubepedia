---
id: VARIABLE-REGISTRY_INGRESS_TLS_SECRET
type: variable
title: registry_ingress_tls_secret
status: active
...
Kubespray variable `registry_ingress_tls_secret` — default `(empty)`. Defined in `roles/kubernetes-apps/registry/defaults/main.yml`. Present in Kubespray
...
```yaml
registry_ingress_tls_secret: (empty)
```
