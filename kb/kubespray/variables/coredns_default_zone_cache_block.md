---
id: VARIABLE-COREDNS_DEFAULT_ZONE_CACHE_BLOCK
type: variable
title: coredns_default_zone_cache_block
status: active
...
Kubespray variable `coredns_default_zone_cache_block` — default `(structured / block value — see source)`. Defined in `roles/kubernetes-apps/ansible/defaults/main.yml`. Present in Kubespray
...
```yaml
coredns_default_zone_cache_block: (structured / block value — see source)
```
