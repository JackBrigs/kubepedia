---
id: VARIABLE-EXTERNAL_ORACLE_AUTH_FINGERPRINT
type: variable
title: external_oracle_auth_fingerprint
status: active
...
Kubespray variable `external_oracle_auth_fingerprint` — default `(empty)`. Defined in `roles/kubernetes-apps/external_cloud_controller/oci/defaults/main.yml`. Present in Kubespray
...
```yaml
external_oracle_auth_fingerprint: (empty)
```
