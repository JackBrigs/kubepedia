...
Kubespray variable `youki_runtime` — default `(structured / block value — see source)`. Defined in `roles/container-engine/cri-o/defaults/main.yml`. Present in Kubespray
...
```yaml
youki_runtime: (structured / block value — see source)
```
