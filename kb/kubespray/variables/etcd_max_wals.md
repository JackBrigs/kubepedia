...
sources:
  - type: code
    ...
    note: "Maximum number of etcd WAL files to retain; default 5"
relations:
  - type: see_also
...
## Summary
Sets etcd's `--max-wals` flag, i.e. the maximum number of write-ahead-log files to retain. Default is `5`.
...
## Compatibility
Kubespray >=v2.29.0 <=v2.31.0 (present in every tag inspected). Related etcd retention variable: `etcd_max_snapshots`.
