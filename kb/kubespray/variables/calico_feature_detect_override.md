---
id: VARIABLE-CALICO_FEATURE_DETECT_OVERRIDE
type: variable
title: calico_feature_detect_override
status: active
...
Kubespray variable `calico_feature_detect_override` — default `(empty)`. Defined in `roles/network_plugin/calico_defaults/defaults/main.yml`. Present in Kubespray
...
```yaml
calico_feature_detect_override: (empty)
```
