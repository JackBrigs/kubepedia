---
id: VARIABLE-REGISTRY_SERVICE_LOADBALANCER_IP
type: variable
title: registry_service_loadbalancer_ip
status: active
...
Kubespray variable `registry_service_loadbalancer_ip` — default `(empty)`. Defined in `roles/kubernetes-apps/registry/defaults/main.yml`. Present in Kubespray
...
```yaml
registry_service_loadbalancer_ip: (empty)
```
