---
id: VARIABLE-CRICTL_SUPPORTED_VERSIONS
type: variable
title: crictl_supported_versions
status: active
...
Kubespray variable `crictl_supported_versions` — default `(structured / block value — see source)`. Defined in `roles/kubespray_defaults/defaults/main/download.yml`. Present in Kubespray
...
```yaml
crictl_supported_versions: (structured / block value — see source)
```
