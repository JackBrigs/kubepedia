...
sources:
  ...
  - type: code
    ...
    url: https://github.com/kubernetes-sigs/kubespray/tree/v2.30.0/roles/etcd/tasks
    note: "etcd_deployment_type in ['host', 'docker', 'kubeadm'] selects install method (both tags)"
relations:
  - type: part_of
...
unchanged across `v2.29.0`–`v2.30.0`.
...
- Kubespray `v2.29.0`–`v2.30.0`: default `host`; accepted `host` | `docker` |
  `kubeadm` (unchanged).
