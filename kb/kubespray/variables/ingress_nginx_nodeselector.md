---
id: VARIABLE-INGRESS_NGINX_NODESELECTOR
type: variable
title: ingress_nginx_nodeselector
status: active
...
Kubespray variable `ingress_nginx_nodeselector` — default `(structured / block value — see source)`. Defined in `roles/kubernetes-apps/ingress_controller/ingress_nginx/defaults/main.yml`. Present in Kubespray
...
```yaml
ingress_nginx_nodeselector: (structured / block value — see source)
```
