---
id: VARIABLE-KREW_DOWNLOAD_URL
type: variable
title: krew_download_url
status: active
...
sources:
  - type: code
    ...
    note: "default: {{ github_url }}/kubernetes-sigs/krew/releases/download/{{ krew_ver…"
relations: []
---
...
Kubespray variable `krew_download_url` — default `{{ github_url }}/kubernetes-sigs/krew/releases/download/{{ krew_ver…`. Defined in `roles/kubespray-defaults/defaults/main/download.yml`. Present in Kubespray
...
```yaml
krew_download_url: {{ github_url }}/kubernetes-sigs/krew/releases/download/{{ krew_ver…
```
