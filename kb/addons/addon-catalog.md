...
index only; `COMPONENT-ADDON_<X>` = a dedicated deep doc exists.
...
|-------|----------|-------|-----|-------|-----|
| vault | HashiCorp vault | 0.32.0 | 1.21.4 | upstream | catalog |
| vault-secrets-webhook | bank-vaults | 1.21.4 | — | upstream | catalog |
| 1password-connect | connect | 1.17.0 | 1.7.3 | upstream | catalog |
| dex | dex (+ userinfo-provider 0.1.0, kubectl-web 0.0.1) | 0.23.0 | — | upstream | catalog |
...
| Addon | Upstream | Chart | App | Class | Doc |
|-------|----------|-------|-----|-------|-----|
| argocd | argo-cd | 8.5.7 | — | upstream | catalog (overlaps [[COMPONENT-ARGOCD]]) |
| gitlab-agent | gitlab-agent | 2.22.1 / 2.26.0 | — | upstream | catalog |
| gitlab-runner | gitlab-runner | 0.81.0 | 18.4.0 | upstream | catalog |
| gitlab-com-runner | gitlab-runner | 0.63.0 | 16.10.0 | upstream | catalog |
| gitlab-ci-control | in-house | 0.0.1 | — | in-house | catalog |
| gitlab-runners-rbac | in-house | 0.0.1 | — | in-house | catalog |
| flagger | flagger | 1.40.0 | — | upstream | catalog |

### Observability

| Addon | Upstream | Chart | App | Class | Doc |
|-------|----------|-------|-----|-------|-----|
| vm | victoria-metrics-k8s-stack (+ vmalert 0.17.0, grafana-operator v5.18.0, oauth2-proxy 8.1.0, helm-exporter 1.2.16) | 0.42.0 | — | upstream | catalog (overlaps [[CONCEPT-OBSERVABILITY_STACK]]) |
| alertmanager | prometheus-community alertmanager | 0.28.0 | — | upstream | catalog |
...
| pyrra | pyrra | 1.1.0 | v0.8.1 | upstream | catalog |
| vector | vector-operator | 0.7.2 | — | upstream | catalog |
| opentelemetry | opentelemetry | — | — | upstream | catalog |
| opentelemetry-operator | opentelemetry-operator (all versions) | — | — | upstream | catalog |
| release-watcher | release-watcher | 0.0.9 | — | upstream | catalog |
| headlamp | headlamp | 0.43.0 | — | upstream | catalog |
| dashboard | kubernetes-dashboard (+ oauth2-proxy 8.1.1) | 7.6.1 | — | upstream | catalog |

### Storage & data

| Addon | Upstream | Chart | App | Class | Doc |
|-------|----------|-------|-----|-------|-----|
| rook-operator | rook-ceph | 1.18.9 | — | upstream | catalog |
| rook-ceph-cluster | rook-ceph-cluster | 1.18.9 | — | upstream | catalog |
| csi-cephfs | ceph-csi-cephfs | 3.14.2 / 3.13.0 | — | upstream | catalog |
| cephbucket | in-house | current / v1.1.35 | 1.0.0 | in-house | catalog |
| lvm-localpv | OpenEBS lvm-localpv | 1.7.0 | — | upstream | catalog |
| snapshotter | external-snapshotter (local) | v6.3.0 | — | upstream | catalog (overlaps [[COMPONENT-SNAPSHOT_CONTROLLER]]) |
| volsync | volsync | 0.15.0 | 3.5.0 | upstream | catalog |
| k8up | k8up (+ k8upcrd 0.1.0) | 4.8.4 | — | upstream | catalog |
| velero | velero | 11.4.0 | 1.17.1 | upstream | catalog (overlaps [[CONCEPT-VELERO_BACKUP]]) |
| zalando-postgres-operator | postgres-operator | 1.14.0 | — | upstream | catalog |
...
| elastic | eck-operator | 3.1.0 | — | upstream | catalog |
...
| eg (envoy gateway) | gateway-helm / gateway-addons-helm | v1.4.1→v1.6.0 | — | upstream | catalog |
| exc (envoy-xds-controller) | envoy-xds-controller (+ dex 0.22.1) | 0.87.0 | — | upstream | catalog |
| exc-stage | envoy-xds-controller (+ dex 0.22.1) | 0.86.0 | — | upstream | catalog (variant of exc) |
| exc-test | envoy-xds-controller (+ dex 0.22.1) | 0.84.0 | — | upstream | catalog (variant of exc) |
| ingress2gateway | ingress2gateway | 0.0.6 | — | upstream | catalog |

### Multi-tenancy, operators & platform

| Addon | Upstream | Chart | App | Class | Doc |
|-------|----------|-------|-----|-------|-----|
| capsule-system | capsule / capsule-proxy | 0.10.9→0.13.3 / 0.9.13→0.13.4 | — | upstream | catalog |
| operator-lifecycle-manager | OLM (in-house packaging) | 0.32.0 | — | upstream | catalog |
...
| Addon | Upstream | Chart | App | Class | Doc |
|-------|----------|-------|-----|-------|-----|
| gpu-operator | NVIDIA gpu-operator | v25.10.1 | — | upstream | catalog |
| kvm-device-plugin | kvm-device-plugin | 1.0.0 | — | upstream | catalog |
| keda | keda | 2.17.2 | — | upstream | catalog |
