...
На каждом из 4 хопов (при обычном апгрейде без teardown) агент (DaemonSet) катится rolling по нодам;
...
Хопы (для каждого: preflight-DaemonSet целевой версии → `cilium_version` + `upgradeCompatibility (cilium_extra_values)`
...
Финал: убрать `upgradeCompatibility` из `cilium_extra_values`, converge, повторить connectivity test.
...
Подтвердить: 1.18.4 стабильна, `upgradeCompatibility (cilium_extra_values)` снят, connectivity зелёный, инцидентов нет.
