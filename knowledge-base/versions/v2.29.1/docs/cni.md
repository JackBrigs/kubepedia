...
# CNI и DNS-стек в v2.29.1

Дайджест документации `docs/` тега v2.29.1. В проекте детально индексируется только CNI **Cilium**; остальные плагины перечислены справочно. Также разобран DNS-стек (CoreDNS / nodelocaldns) и утилита проверки сети netchecker.

Связанные заметки: [[versions/v2.29.1/variables/cni|Переменные CNI]] · [[versions/v2.29.1/ansible-tags|Ansible-теги]] · [[versions/v2.29.1/README|Срез v2.29.1]].
...
- Версия Cilium — `cilium_version` (пример в доке: `"1.18.4"`).
...
> Примечание: режимы маршрутизации (tunnel / native routing) и подробные значения по умолчанию в тексте `docs/CNI/cilium.md` тега v2.29.1 отдельными разделами не описаны — конкретные дефолты и режим датаплейна см. в [[versions/v2.29.1/variables/cni|переменных CNI]] (роль `roles/network_plugin/cilium/defaults`).
...
- `docs/CNI/cilium.md` (v2.29.1)
- `docs/CNI/cni.md` (v2.29.1)
- `docs/advanced/dns-stack.md` (v2.29.1)
- `docs/advanced/netcheck.md` (v2.29.1)
...
- Репозиторий тега: https://github.com/kubernetes-sigs/kubespray/tree/v2.29.1
