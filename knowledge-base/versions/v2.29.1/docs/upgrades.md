...
Дайджест построен строго по документации тега `v2.29.1` (commit `0c6a295`): `docs/operations/upgrades.md` и `docs/upgrades/migrate_docker2containerd.md`. Это выжимка, а не дословный перевод; имена переменных, тегов и команд приведены точно.
...
- Это ровно тот принцип, на котором построена база знаний: последовательность `v2.29.1 → v2.30.0 → v2.31.0 → ...` без пропусков.
...
Подробности по тегам запуска: [[versions/v2.29.1/ansible-tags|Ansible-теги запуска v2.29.1]].
...
Связанные переменные: [[versions/v2.29.1/variables/etcd|Переменные etcd v2.29.1]].
...
- `docs/operations/upgrades.md` (тег v2.29.1)
- `docs/upgrades/migrate_docker2containerd.md` (тег v2.29.1)
