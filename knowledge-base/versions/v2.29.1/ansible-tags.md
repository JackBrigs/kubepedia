...
git_commit: 0c6a295
source_type: code
source_path: versions/v2.29.1/ansible-tags.yaml
...
# Ansible-теги запуска Kubespray v2.29.1
...
- **Всего тегов в v2.29.1:** 124 (извлечены из `playbooks/*.yml` и `roles/*/tasks/**`, полнота сверена программно).
...
| `cni` | cluster.yml, upgrade-cluster.yml | safe | ВНИМАНИЕ: по коду тега (приоритет у кода над docs/ansible/ansible.md, где тег описан как «CNI plugins for Network Plugins») тег cni в v2.29.1 навешан единств… |
...
Во всех случаях приоритет отдан коду тега v2.29.1 (раздел 5.1 CLAUDE.md); расхождения зафиксированы в поле `notes` соответствующих записей YAML.
...
- **`scheduler_plugins` на K8s 1.29+ ничего не делает.** Роль подключается только при `kube_major_version < 1.29`; при дефолтной 1.33.7 тег фактически пуст.
