...
git_commit: 0c6a295
source_type: code
source_path: versions/v2.29.1/variables/addons.yaml
...
# Переменные аддонов (kubernetes-apps) в Kubespray v2.29.1

Справочник переменных дополнительных компонентов, разворачиваемых ролями `roles/kubernetes-apps/*` на теге `v2.29.1` (коммит `0c6a295`).

**Источник истины** — машиночитаемый справочник [[versions/v2.29.1/variables/addons|addons.yaml]] (366 переменных). Эта заметка — человекочитаемое изложение.

Назад к срезу: [[versions/v2.29.1/README|Срез v2.29.1]]

## Обзор

В v2.29.1 через `kubernetes-apps` доступны следующие аддоны:
...
- **Helm** (установка CLI на control plane) и роль `helm-apps` (установка Helm-чартов, например kubelet-csr-approver);
...
- **MetalLB**;
- **Argo CD**;
- **CRD**: Gateway API, Prometheus Operator CRDs (роль `common_crds`);
...
- `roles/kubernetes-apps/container_engine_accelerator` (NVIDIA GPU) — не проиндексировано;
...
| Prometheus Operator CRD | `prometheus_operator_crds_enabled` | `false` | `roles/kubespray_defaults/defaults/main/main.yml` |
...
| `coredns_replicas` | `2` | реплики при выключенном dns-autoscaler |
...
Сам флаг `enable_nodelocaldns` относится к справочнику k8s-cluster (kubespray_defaults), здесь описаны только ресурсы/настройки подов.
...
| `ingress_nginx_host_network` | `false` | hostNetwork |
...
| `ingress_nginx_tolerations` | `[]` | tolerations подов |
...
Пулы адресов (IPAddressPool, L2/BGP-advertisement) в defaults роли v2.29.1 не задаются — настраиваются пользователем через инвентарь (см. документацию `docs/ingress/metallb.md` тега).
...
| OpenStack | `v1.32.0` | креды из окружения `OS_*`; `external_openstack_lbaas_member_subnet_id` требует заданного пользователем `external_openstack_lbaas_subnet_id` |
...
- Роли без собственных `defaults/`: `cluster_roles`, `common_crds/prometheus_operator_crds`, `csi_driver/csi_crd`, `helm-apps` (переменные в `vars/`).
