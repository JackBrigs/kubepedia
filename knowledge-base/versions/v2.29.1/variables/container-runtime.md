...
git_commit: 0c6a295
source_type: code
source_path: versions/v2.29.1/variables/container-runtime.yaml
...
# Container runtime в Kubespray v2.29.1

Справочник переменных container runtime для тега `v2.29.1` (commit `0c6a295`).
**Источник истины** — машинно-читаемый файл [[versions/v2.29.1/variables/container-runtime|container-runtime.yaml]] (85 переменных); эта заметка — его человекочитаемое изложение.
...
|---|---|
| `containerd_version` | первый ключ `containerd_archive_checksums['amd64']` → **2.1.5** в этом теге |
...
- `containerd_registry_auth` — `[]`; список registry/username/password. Внимание: в теге используется только легаси-шаблоном `config-v1.toml.j2`, то есть при дефолтном containerd 2.1.5 не применяется (в YAML помечено `reliability: unconfirmed`).
...
|---|---|---|---|
| runc | 1.3.4 (`runc_version`) | `runc_bin_dir: {{ bin_dir }}`, `runc_package_name: runc` | роль удаляет пакетный runc и ставит бинарник с GitHub |
...
В теге v2.29.1 в `roles/container-engine/` существуют, но по переменным в этом справочнике **не разобраны**:
