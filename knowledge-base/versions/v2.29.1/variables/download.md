...
# Механизм загрузки (download) в Kubespray v2.29.1

Часть среза [[versions/v2.29.1/README|Срез v2.29.1]]. Источник истины — парный YAML-справочник [[versions/v2.29.1/variables/download|download.yaml]]; эта заметка — его человекочитаемое изложение.

**Важно:** в теге v2.29.1 каталога `roles/download/defaults/` **нет** — роль `download` содержит только `meta/`, `tasks/`, `templates/`. Все переменные механизма загрузки определены в `roles/kubespray_defaults/defaults/main/download.yml` и попадают в область видимости всех ролей через роль `kubespray_defaults`.
...
Полный перечень (66 переменных с дословными Jinja-значениями) — в [[versions/v2.29.1/variables/download|download.yaml]].
