...
git_commit: 0c6a295
source_type: code
source_path: versions/v2.29.1/variables/etcd.yaml
...
# Переменные etcd — Kubespray v2.29.1

Человекочитаемая заметка к машиночитаемому справочнику `etcd.yaml` (65 переменных).
**Источник истины — YAML-справочник** `versions/v2.29.1/variables/etcd.yaml`; эта заметка — его изложение.
Назад к срезу: [[versions/v2.29.1/README|Срез v2.29.1]].

Источники в коде тега `v2.29.1` (commit `0c6a295`):
...
## Типы развёртывания etcd в v2.29.1
...
> Важно: переменная `etcd_kubeadm_enabled` в коде v2.29.1 **не существует** — она
...
|---|---|---|
| `etcd_version` | `{{ etcd_supported_versions[kube_major_version] }}` | Для K8s 1.31–1.33 выбирается новейшая 3.5.x из checksums — фактически **3.5.25** (`roles/kubespray_defaults/vars/main/main.yml:14`, `vars/main/checksums.yml:477`) |
