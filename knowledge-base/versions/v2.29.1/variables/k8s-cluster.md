...
git_commit: 0c6a295
source_type: code
source_path: versions/v2.29.1/variables/k8s-cluster.yaml
...
# Переменные ядра кластера — Kubespray v2.29.1

Справочник переменных ядра Kubernetes-кластера для Kubespray **v2.29.1** (коммит `0c6a295`): версия Kubernetes, сеть и подсети, kube-apiserver, kube-controller-manager, kube-scheduler, kubelet, kube-proxy, DNS, сертификаты, kubeadm и подготовка ОС.
...
Источники (пути относительно корня репозитория kubespray на теге v2.29.1):
...
|---|---|---|
| `kube_version` | `{{ (kubelet_checksums['amd64'] \| dict2items)[0].key }}` | Версия Kubernetes; на теге v2.29.1 фактически резолвится в **1.33.7** |
| `kube_version_min_required` | последний ключ kubelet_checksums | Минимально поддерживаемая версия; на v2.29.1 — **1.31.0** |
