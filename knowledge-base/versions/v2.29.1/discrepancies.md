...
# Расхождения sample-inventory ↔ roles/*/defaults (v2.29.1)
...
- **defaults:** `{{ lookup('env', 'CI_PROJECT_URL') == 'https://gitlab.com/kargo-ci/kubernetes-sigs-kubespray' }}`, `roles/kubespray_defaults/defaults/main/download.yml`
- **Характер:** расхождение формы, не поведения. В defaults значение вычисляется по переменной окружения CI и истинно только внутри CI Kubespray; вне CI эффективное значение совпадает (`false`). Sample форсирует `false` безусловно.
- **Приоритет:** за кодом. При запуске в CI defaults дал бы `true`, но пользовательский `group_vars` переопределяет его на `false`.

Это единственное расхождение среди действующих значений (по всем 5 срезам).
...
Связанные справочники: [[versions/v2.29.1/inventory/k8s-cluster|inventory/k8s-cluster]] · [[versions/v2.29.1/inventory/all|inventory/all]] · [[versions/v2.29.1/inventory/cni-cilium|inventory/cni-cilium]] · [[versions/v2.29.1/inventory/cloud-providers|inventory/cloud-providers]] · [[versions/v2.29.1/inventory/addons|inventory/addons]]
