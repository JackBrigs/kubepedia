...
# Общие настройки инвентаря (group_vars/all) — v2.29.1
...
Зафиксировано одно расхождение **формы** (не поведения):

- **`unsafe_show_logs`** — в sample жёстко `false`, в
  `roles/kubespray_defaults/defaults/main/download.yml` вычисляется по CI-переменной
  окружения (`true` только в CI Kubespray). Вне CI эффективное значение совпадает
  (`false`). Приоритет за кодом, но пользовательский group_vars переопределяет defaults
  роли и форсирует `false`.
