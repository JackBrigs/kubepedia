...
# Механизм загрузки (download) в Kubespray v2.30.0

Часть среза [[versions/v2.30.0/README|Срез v2.30.0]]. Источник истины — парный YAML-справочник [[versions/v2.30.0/variables/download|download.yaml]]; эта заметка — его человекочитаемое изложение.

**Важно:** в теге v2.30.0 каталога `roles/download/defaults/` **нет** — роль `download` содержит только `meta/`, `tasks/`, `templates/`. Все переменные механизма загрузки определены в `roles/kubespray_defaults/defaults/main/download.yml` и попадают в область видимости всех ролей через роль `kubespray_defaults`.

## Как устроен механизм

Центральный элемент — словарь `downloads` (~80 элементов: etcd, cni, kubelet, kubectl, kubeadm, crictl, containerd, runc, cilium, coredns, helm, addons, csi-драйверы и т.д.). Каждый элемент описывает **либо файл** (`file: true` + `url`, `dest`, `checksum`, `unarchive`), **либо контейнерный образ** (`container: true` + `repo`, `tag`), а также условие `enabled` и целевые группы хостов `groups`. Недостающие поля дополняются из `download_defaults`. Роль `download` (`roles/download/tasks/`) обходит этот словарь и выполняет загрузки.
...
| `unsafe_show_logs` | `false` (вне CI Kubespray) | Отладочный вывод задач загрузки |
...
Полный перечень (66 переменных с дословными Jinja-значениями) — в [[versions/v2.30.0/variables/download|download.yaml]]. Разрешённые версии компонентов — в [[versions/v2.30.0/components|components.yaml]].
