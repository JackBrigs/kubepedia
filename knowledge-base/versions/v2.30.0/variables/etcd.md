...
git_commit: f4ccdb5
source_type: code
source_path: versions/v2.30.0/variables/etcd.yaml
...
# Переменные etcd — v2.30.0

Человекочитаемая заметка к машинному справочнику переменных etcd. Источник истины — YAML-файл `etcd.yaml`; здесь приведены обзор и ключевые таблицы. Все данные извлечены строго из кода тега `v2.30.0` (commit `f4ccdb5`).

Связанный срез: [[versions/v2.30.0/README|Срез v2.30.0]]

## Источники

- `roles/etcd_defaults/defaults/main.yml` — основной набор defaults роли etcd
...
Всего задокументировано 64 переменных.
...
| Переменная | Значение по умолчанию | Назначение |
| --- | --- | --- |
| `etcd_version` | `{{ etcd_supported_versions[kube_major_version] }}` | Версия etcd (для v2.30.0 — 3.5.26, старшая < 3.6) |
...
- `etcd_version` для v2.30.0 выбирается из `etcd_supported_versions` (`roles/kubespray_defaults/vars/main/main.yml`) как старшая версия ветки `3.5.x` (< 3.6). На момент тега это `3.5.26`.
