...
# Sample-inventory: inventory.ini (v2.30.0)

Разбор `inventory/sample/inventory.ini` — файла описания хостов и групп тега v2.30.0 (commit `f4ccdb5`). Ссылка на срез: [[versions/v2.30.0/README|Срез v2.30.0]].

## Топология sample

Комментарий в начале файла описывает конфигурацию: **HA-топология со stacked etcd** (etcd на тех же нодах, что и control plane) и **3 рабочих узла**. Все хосты в sample закомментированы — это шаблон, который пользователь заполняет своими нодами. Содержимое файла на теге v2.30.0 идентично v2.29.1.
...
| Группа | Назначение |
|---|---|
| `k8s_cluster` | Агрегирующая группа: `kube_control_plane` + `kube_node`. Именно к ней привязаны group_vars из `group_vars/k8s_cluster/` (k8s-cluster.yml, kube_control_plane.yml, addons.yml). Формируется на уровне статической групповой структуры Kubespray, а не в sample.ini |
...
Привязка group_vars: файлы `inventory/sample/group_vars/k8s_cluster/*.yml` применяются к группе `k8s_cluster`, то есть ко всем control plane и worker узлам одновременно. Файл `kube_control_plane.yml`, несмотря на имя, лежит в каталоге `group_vars/k8s_cluster/` и по умолчанию тоже применяется к группе `k8s_cluster`.
...
Значения переменных кластера разбираются в [[versions/v2.30.0/inventory/k8s-cluster|k8s-cluster.yaml]] и [[versions/v2.30.0/inventory/addons|addons.yaml]]. Данный файл описывает только структуру групп хостов.
