...
# Установка и базовая эксплуатация Kubespray v2.27.0

Дайджест по установке и первичной эксплуатации кластера, собранный строго из каталога `docs/` тега `v2.27.0` (commit `9ec9b3a`). Дословные команды, имена переменных и номера портов сохранены как в оригинале.

Связанные срезы базы: [[versions/v2.27.0/ansible-tags|Ansible-теги]], [[versions/v2.27.0/variables/k8s-cluster|Переменные ядра]], [[versions/v2.27.0/inventory/k8s-cluster|Inventory ядра]].
...
**Важно:** extra vars стоит применять только для переопределения внутренних переменных Kubespray (`roles/vars/`), которые не являются частью публичного интерфейса и могут меняться без предупреждения. Подробнее о переменных — [[versions/v2.27.0/variables/k8s-cluster|Переменные ядра]] (источник `docs/ansible/vars.md`).
...
Kubespray поддерживает выборочный запуск через `--tags` / `--skip-tags`. Полный справочник тегов и их поведения — [[versions/v2.27.0/ansible-tags|Ansible-теги]]. Примеры из `docs/ansible/ansible.md`:
...
Примечание: поддержка **Mitogen** объявлена устаревшей (deprecated) — см. [[versions/v2.27.0/docs/proxy|Прокси и Mitogen]].
...
Примечание: команды туториала используют `git checkout release-2.17` и образы с K8s v1.17.9 — это исторический пример из документации, а не версии, поставляемые тегом v2.27.0. Актуальные версии компонентов — см. [[versions/v2.27.0/components|Компоненты v2.27.0]].

---

## Источники

Все файлы — из каталога `docs/` тега `v2.27.0` (commit `9ec9b3a`):
...
Назад: [[versions/v2.27.0/README|Срез v2.27.0]]
