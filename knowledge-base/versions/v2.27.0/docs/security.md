...
# Безопасность кластера в v2.27.0

Дайджест по усилению безопасности (hardening) и шифрованию секретов в покое (encryption at rest). Все переменные и значения взяты строго из документации тега `v2.27.0` (commit `9ec9b3a`).
...
| `remove_anonymous_access` | `true` | Удаление rolebinding `kubeadm:bootstrap-signer-clusterinfo` (анонимный доступ к cluster-info); не отключает саму анонимную аутентификацию API |
...
- В `hardening.yaml` тега v2.27.0 задано `etcd_deployment_type: kubeadm` (etcd как static pod на control plane). Это отличается от рекомендации hardening в более поздних версиях, где используется `etcd_deployment_type: host`. Здесь фиксируется точное значение из документа v2.27.0.
...
Пары `kubelet_systemd_hardening` + `kubelet_secure_addresses` настраивают минимальный firewall на узле (требуется cgroups v2).
...
Замечание: для AppArmor-based ОС (Debian/Ubuntu) можно раскомментировать в файле фичи `AppArmor` в `kube_apiserver_feature_gates` / `kube_controller_feature_gates` / `kube_scheduler_feature_gates`.
...
Источник: `docs/operations/encrypting-secret-data-at-rest.md`. Смысл: `kube-apiserver` шифрует данные до записи в `etcd`, поэтому в самом `etcd` данные нечитаемы. По состоянию документа (22.02.2022) поддерживаются 5 провайдеров шифрования.
...
- [[versions/v2.27.0/variables/k8s-cluster|Переменные ядра кластера]]
- [[versions/v2.27.0/variables/etcd|Переменные etcd]]
- [[versions/v2.27.0/docs/troubleshooting|Требования и troubleshooting]]

## Источники

- `docs/operations/hardening.md` — <https://github.com/kubernetes-sigs/kubespray/blob/v2.27.0/docs/operations/hardening.md>
- `docs/operations/encrypting-secret-data-at-rest.md` — <https://github.com/kubernetes-sigs/kubespray/blob/v2.27.0/docs/operations/encrypting-secret-data-at-rest.md>
