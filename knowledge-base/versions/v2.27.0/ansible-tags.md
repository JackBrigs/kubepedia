...
git_commit: 9ec9b3a
source_type: code
source_path: versions/v2.27.0/ansible-tags.yaml
...
# Ansible-теги запуска Kubespray v2.27.0
...
- **Всего тегов в v2.27.0:** 127 (извлечены из `playbooks/*.yml` и `roles/*/tasks/**`, полнота сверена программно).
...
> ⚠️ **Отличия версии v2.27.0 от v2.29.1** (подробности — в поле `notes` соответствующих записей YAML):
> - Роль контрол-плейна в `cluster.yml` помечена тегом **`master`** (в v2.29.1 — `control-plane`). Тег `control-plane` в v2.27.0 покрывает лишь вспомогательные задачи (каталоги + серийники etcd-сертификатов).
...
# переустановить/обновить контрол-плейн (в v2.27.0 — именно master)
...
| `control-plane` | cluster.yml, upgrade-cluster.yml, scale.yml | safe | ВНИМАНИЕ (приоритет у кода над docs): в v2.27.0 тег control-plane НЕ разворачивает роль контрол-плейна (её разворачивает тег master, см. |
...
| `master` | cluster.yml, upgrade-cluster.yml | risky | Основной тег развёртывания/переконфигурации контрол-плейна в v2.27.0. |
...
| `cni` | cluster.yml, upgrade-cluster.yml | safe | ВНИМАНИЕ: по коду тега (приоритет у кода над docs/ansible/ansible.md, где тег описан как «CNI plugins for Network Plugins») тег cni в v2.27.0 навешан ед… |
...
Во всех случаях приоритет отдан коду тега v2.27.0 (раздел 5.1 CLAUDE.md); расхождения зафиксированы в поле `notes` соответствующих записей YAML.
...
- **`cilium` — статические манифесты.** В v2.27.0 Cilium разворачивается через `kubectl apply` набора манифестов (ConfigMap, DaemonSet, Deployment cilium-operator, RBAC), а не через `cilium-cli`. Минимальная версия `cilium_min_version_required = 1.10`.
...
- **Только в v2.27.0:** `weave` (CNI Weave Net), `cephfs-provisioner` и `rbd-provisioner` (внешние Ceph-провиженеры), `cloud-provider` (assert-проверки настроек облака), `etchosts` (правка `/etc/hosts`), `krew` (менеджер плагинов kubectl). Все удалены в поздних релизах.
