...
# Sample-inventory: k8s-cluster.yml (v2.27.0)

Разбор главного пользовательского файла настроек кластера `k8s-cluster.yml` из
группы `k8s_cluster`. Источник истины — [[versions/v2.27.0/inventory/k8s-cluster|k8s-cluster.yaml (справочник)]];
здесь человекочитаемое изложение. Аддоны разобраны отдельно:
[[versions/v2.27.0/inventory/addons|addons.yaml]].

Ссылка на срез: [[versions/v2.27.0/README|Срез v2.27.0]].
...
Всего в справочнике: **129 переменных**, из них реально задано (`is_set: true`) —
**59**, закомментированных примеров — **70**.

> [!important] Отличия v2.27.0 от v2.29.1
> - **`kube_version: v1.31.4` задан прямо в sample** (в v2.29.1 в этом файле его нет). Значение совпадает с defaults роли.
...
> - **Отдельного файла `kube_control_plane.yml` НЕТ**. Резервирование ресурсов для control plane задаётся здесь же переменными `kube_master_*` и `system_master_*` (в v2.29.1 переименовано и вынесено в `kube_control_plane.yml`).

## Ключевые заданные настройки (сеть и подсети)

| Переменная | Значение в sample | Назначение |
|---|---|---|
| `kube_version` | `v1.31.4` | Версия Kubernetes |
...
## Резервирование ресурсов (в одном файле)

В отличие от v2.29.1, в v2.27.0 всё резервирование ресурсов собрано в `k8s-cluster.yml`
(все переменные закомментированы). Для рабочих нод — `kube_*_reserved` /
`system_*_reserved`; для узлов control plane — отдельные переменные с суффиксом `master`:

| Переменная (control plane) | Пример | Переменная (worker) | Пример |
|---|---|---|---|
| `kube_master_memory_reserved` | `512Mi` | `kube_memory_reserved` | `256Mi` |
| `kube_master_cpu_reserved` | `200m` | `kube_cpu_reserved` | `100m` |
| `kube_master_ephemeral_storage_reserved` | `2Gi` | `kube_ephemeral_storage_reserved` | `2Gi` |
| `kube_master_pid_reserved` | `"1000"` | `kube_pid_reserved` | `"1000"` |
| `system_master_memory_reserved` | `256Mi` | `system_memory_reserved` | `512Mi` |
| `system_master_cpu_reserved` | `250m` | `system_cpu_reserved` | `500m` |
| `system_master_ephemeral_storage_reserved` | `2Gi` | `system_ephemeral_storage_reserved` | `2Gi` |

Аналога `system_master_pid_reserved` в v2.27.0 нет.
...
`kube_version: v1.31.4` и `container_manager: containerd`). Подробности:
[[versions/v2.27.0/discrepancies|Расхождения inventory vs defaults]]. По правилу
