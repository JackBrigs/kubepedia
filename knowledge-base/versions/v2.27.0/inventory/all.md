...
# Общие настройки инвентаря (group_vars/all) — v2.27.0
...
[[versions/v2.27.0/inventory/all|all.yaml]] в этом же каталоге; данная заметка — человекочитаемое изложение.

Срез версии: [[versions/v2.27.0/README|Срез v2.27.0]].
...
> В `all.yml` тега v2.27.0 **отсутствует** переменная `sysctl_ignore_unknown_keys`
...
> - В v2.27.0 присутствует `krew_download_url` (при `krew_enabled: true`).
...
`container_manager` (наследуемый) и, что важно для v2.27.0, у `unsafe_show_logs`.

> [!important] unsafe_show_logs в v2.27.0
> В отличие от v2.29.1, в v2.27.0 default роли `unsafe_show_logs` — жёсткий `false`
...
[[versions/v2.27.0/discrepancies|Расхождения inventory vs defaults]].
