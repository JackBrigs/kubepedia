...
git_commit: 9ec9b3a
source_type: code
source_path: versions/v2.27.0/variables/cni.yaml
...
# Переменные CNI (Cilium) — Kubespray v2.27.0

Человекочитаемая заметка к машиночитаемому справочнику `cni.yaml`.
**Источник истины — YAML-справочник** `versions/v2.27.0/variables/cni.yaml`.
...
`network_plugin/cni` раскладывает базовые CNI-бинарники (`containernetworking/plugins`, `cni_version: v1.4.0`).

## Ключевое отличие v2.27.0: Cilium разворачивается манифестами

В v2.27.0 Cilium ставится **шаблонами-манифестами роли**, а не через helm/cilium-cli (как в v2.29.1).
...
| `cilium_cli_version` | `v0.16.0` | явный литерал |
...
- [[versions/v2.27.0/variables/k8s-cluster|k8s-cluster.yaml]] — переменные ядра сети (`kube_pods_subnet`, `kube_network_node_prefix`)
- [[versions/v2.27.0/variables/download|download.yaml]] — механизм загрузок
- [[versions/v2.27.0/variables/etcd|etcd.yaml]] — etcd используется Cilium при `identity_allocation_mode: kvstore`
