...
git_commit: 9ec9b3a
source_type: code
source_path: versions/v2.27.0/variables/container-runtime.yaml
...
# Переменные container runtime (containerd) — Kubespray v2.27.0

Человекочитаемая заметка к машиночитаемому справочнику `container-runtime.yaml`.
**Источник истины — YAML-справочник** `versions/v2.27.0/variables/container-runtime.yaml`.
...
|---|---|---|---|
| containerd | `containerd_version` | **1.7.24** | явный литерал (в v2.29.1 вычислялся из checksums, был 2.1.5) |
| runc | `runc_version` | **v1.2.3** | явный литерал |
...
- `containerd_runc_runtime.options` в v2.27.0 называются **`systemdCgroup` / `binaryName`** (в v2.29.1 — `SystemdCgroup` / `BinaryName`).
...
- [[versions/v2.27.0/variables/download|download.yaml]] — механизм загрузок и контрольные суммы
- [[versions/v2.27.0/variables/etcd|etcd.yaml]] — `etcd_deployment_type: docker` тоже требует движка
