...
# Sample-inventory: addons.yml (v2.28.0)

Разбор `inventory/sample/group_vars/k8s_cluster/addons.yml` (248 строк) — файла включения дополнительных компонентов кластера. Источник истины — [[versions/v2.28.0/inventory/addons|addons.yaml (справочник)]].

Ссылка на срез: [[versions/v2.28.0/README|Срез v2.28.0]].

Всего в справочнике: **93 переменные**, из них реально задано (`is_set: true`) — **17**, закомментированных примеров — **76**.

> [!note] Отличие от v2.29.1
> В v2.28.0 в блоке Local Path Provisioner присутствует закомментированный пример `local_path_provisioner_image_tag: "v0.0.24"`, удалённый в v2.29.1 (там 92 переменные).
