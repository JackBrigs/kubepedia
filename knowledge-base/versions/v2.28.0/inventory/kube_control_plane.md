...
# Sample-inventory: kube_control_plane.yml (v2.28.0)

Разбор файла `inventory/sample/group_vars/k8s_cluster/kube_control_plane.yml` тега **v2.28.0** (commit `63cdf87`). Источник истины — [[versions/v2.28.0/inventory/kube_control_plane|kube_control_plane.yaml]]; здесь человекочитаемое изложение.
...
Все переменные закомментированы, значений не переопределяют. Самостоятельных одноимённых переменных в `roles/*/defaults` нет — резервирование настраивается пользователем на уровне group_vars. **Расхождений не обнаружено.** Подробности: [[versions/v2.28.0/discrepancies|Расхождения inventory vs defaults]].

## Связанные заметки

- [[versions/v2.28.0/inventory/k8s-cluster|k8s-cluster.yaml (справочник)]]
