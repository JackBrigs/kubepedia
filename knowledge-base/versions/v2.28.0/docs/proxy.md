...
Дайджест документации по настройке прокси строго с тега `v2.28.0` (commit `63cdf87`). Файлы `docs/advanced/proxy.md` и `docs/advanced/mitogen.md` идентичны версии v2.29.1.
...
генерируется в `roles/kubespray_defaults/tasks/no_proxy.yml`.
...
- По умолчанию все worker-узлы входят в `no_proxy`. Из-за этого при добавлении
  или удалении worker-узлов container engine перезапускается на всех узлах
  (перезапускаются все pod'ы), так как меняется значение no_proxy.
...
в одной из будущих версий. К проксированию не относится — приведён как средство
ускорения с пометкой об устаревании.

## Источники

- `docs/advanced/proxy.md` — https://github.com/kubernetes-sigs/kubespray/blob/v2.28.0/docs/advanced/proxy.md
- `docs/advanced/mitogen.md` — https://github.com/kubernetes-sigs/kubespray/blob/v2.28.0/docs/advanced/mitogen.md

Связанные заметки: [[versions/v2.28.0/docs/offline|Оффлайн-развёртывание и зеркала]] · [[versions/v2.28.0/variables/download|Переменные download]] · [[versions/v2.28.0/README|Срез v2.28.0]]
