...
# Управление узлами и восстановление control plane в v2.28.0

Дайджест построен строго по документации тега `v2.28.0` (commit `63cdf87`): `docs/operations/nodes.md` и `docs/operations/recover-control-plane.md`. Выжимка, не дословный перевод; имена плейбуков, переменных и команд приведены точно.

> Отличие от v2.29.1: в v2.28.0 раздел про удаление первого control plane состоит из двух частей — вверху файла раздел «Limitation: Removal of first kube_control_plane and etcd-master» (примеры инвентаря в формате **YAML**) и ниже отдельный раздел «Replacing a first control plane node» (переупорядочивание инвентаря в формате **INI**). В v2.29.1 эти части были объединены в один раздел с YAML-примерами. Операционное содержание идентично.

## Ключевые переменные и плейбуки

- Плейбуки: `scale.yml` (добавление worker), `cluster.yml` (добавление control plane / etcd, перегенерация конфигов), `upgrade-cluster.yml`, `remove-node.yml` (удаление), `playbooks/facts.yml` (обновление кэша фактов), `recover-control-plane.yml` (восстановление).
...
Связанные срезы: [[versions/v2.28.0/ansible-tags|Ansible-теги запуска v2.28.0]] (reset, pre-remove, post-remove), [[versions/v2.28.0/variables/etcd|Переменные etcd v2.28.0]].

## Источники

- `docs/operations/nodes.md` (тег v2.28.0)
- `docs/operations/recover-control-plane.md` (тег v2.28.0)
