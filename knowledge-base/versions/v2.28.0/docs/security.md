...
# Безопасность кластера в v2.28.0

Дайджест по усилению безопасности (hardening) и шифрованию секретов в покое (encryption at rest). Все переменные и значения взяты строго из документации тега `v2.28.0` (commit `63cdf87`).

> Отличия `docs/operations/hardening.md` от v2.29.1: (1) в примере v2.28.0 задано `etcd_deployment_type: kubeadm` (в v2.29.1 заменено на `etcd_deployment_type: host` с пояснением о безопасности выделенного etcd); (2) в v2.28.0 присутствуют закомментированные строки под AppArmor-based OS (feature gate `AppArmor=true` для apiserver / controller-manager / scheduler), удалённые в v2.29.1; (3) в v2.28.0 **нет** переменной `kubelet_static_pod_path: ""` (добавлена в v2.29.1). Файл `encrypting-secret-data-at-rest.md` идентичен v2.29.1.
...
Для AppArmor-based ОС (Debian/Ubuntu) в примере v2.28.0 приведена закомментированная строка `# kube_apiserver_feature_gates: ['AppArmor=true']` — включается расскомментированием.
...
Для AppArmor-based ОС в примере v2.28.0 приведена закомментированная альтернатива `# kube_controller_feature_gates: ["RotateKubeletServerCertificate=true", "AppArmor=true"]`.
...
Для AppArmor-based ОС в примере v2.28.0 приведена закомментированная строка `# kube_scheduler_feature_gates: ["AppArmor=true"]`.

### etcd

- В примере `hardening.yaml` тега v2.28.0 задано `etcd_deployment_type: kubeadm`. (В v2.29.1 это значение заменено на `etcd_deployment_type: host` с пояснением, что запуск etcd на выделенных хостах вне кластера Kubernetes — наиболее защищённый вариант.)
...
Примечание: переменная `kubelet_static_pod_path: ""` (отключение staticPodPath на worker-узлах) в примере v2.28.0 **отсутствует** — она была добавлена в v2.29.1.
...
- [[versions/v2.28.0/variables/etcd|Переменные etcd]]
- [[versions/v2.28.0/docs/troubleshooting|Требования и troubleshooting]]

## Источники

- `docs/operations/hardening.md` — <https://github.com/kubernetes-sigs/kubespray/blob/v2.28.0/docs/operations/hardening.md>
- `docs/operations/encrypting-secret-data-at-rest.md` — <https://github.com/kubernetes-sigs/kubespray/blob/v2.28.0/docs/operations/encrypting-secret-data-at-rest.md>
