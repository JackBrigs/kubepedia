...
# Sample-inventory: k8s-cluster.yml и kube_control_plane.yml (v2.29.0)

Разбор двух файлов группы `k8s_cluster` из `inventory/sample/group_vars/`. Это главные пользовательские настройки кластера. Источник истины — [[versions/v2.29.0/inventory/k8s-cluster|k8s-cluster.yaml (справочник)]]; здесь человекочитаемое изложение.

Ссылка на срез: [[versions/v2.29.0/README|Срез v2.29.0]].

## Что это за файлы

- **`k8s-cluster.yml`** (372 строки) — основные настройки кластера: каталоги, сеть, DNS, kube-proxy, API-сервер, рантайм, резервирование ресурсов, сертификаты. Большинство «боевых» переменных, которые правит пользователь, находятся здесь.
- **`kube_control_plane.yml`** — резервирование ресурсов (CPU/память/storage/PID) **только для узлов control plane**. В sample все переменные закомментированы (примеры).
...
Всего в справочнике: **128 переменных**, из них реально задано (`is_set: true`) — **57**, закомментированных примеров — **71**.

## Ключевые заданные настройки (сеть и подсети)

| Переменная | Значение в sample | Назначение |
|---|---|---|
| `kube_network_plugin` | `calico` | CNI по умолчанию (варианты: cilium, calico, kube-ovn, flannel, cni, cloud) |
...
Секции `kube_reserved` и `system_reserved` в `k8s-cluster.yml` **закомментированы** (примеры для рабочих нод). Отдельный файл `kube_control_plane.yml` содержит **закомментированные** примеры резервирования для узлов control plane — с иными значениями:

| Переменная | Пример (control plane) | Пример (worker, в k8s-cluster.yml) |
|---|---|---|
...
`kube_control_plane.yml` также добавляет `system_pid_reserved` (пример `"1000"`), которого нет в примерах `k8s-cluster.yml`.
...
Все реально заданные (`is_set: true`) переменные проверены против `roles/*/defaults` (этап 2). **Расхождений не обнаружено** — значения sample совпадают с defaults. Подробности: [[versions/v2.29.0/discrepancies|Расхождения inventory vs defaults]]. По правилу раздела 6.2 CLAUDE.md при расхождении приоритет имеет код ролей.
