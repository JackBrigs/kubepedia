...
# Общие настройки инвентаря (group_vars/all) — v2.29.0
...
Расхождений действующих значений нет: `unsafe_show_logs` в sample задан `false`, и
default роли `roles/kubespray_defaults/defaults/main/download.yml` в теге v2.29.0 —
тоже литерал `false` (в v2.29.1 default этой переменной был заменён на выражение по
CI-переменной окружения, что породило расхождение формы; в v2.29.0 такого расхождения
ещё нет).
