...
git_commit: 9991412
source_type: code
source_path: versions/v2.29.0/variables/addons.yaml
...
# Переменные аддонов (kubernetes-apps) в Kubespray v2.29.0

Справочник переменных дополнительных компонентов, разворачиваемых ролями `roles/kubernetes-apps/*` на теге `v2.29.0` (коммит `9991412`).

**Источник истины** — машиночитаемый справочник [[versions/v2.29.0/variables/addons|addons.yaml]] (367 переменных). Эта заметка — человекочитаемое изложение.

Назад к срезу: [[versions/v2.29.0/README|Срез v2.29.0]]

## Обзор

В v2.29.0 через `kubernetes-apps` доступны следующие аддоны:
...
Пулы адресов (IPAddressPool, L2/BGP-advertisement) в defaults роли v2.29.0 не задаются — настраиваются пользователем через инвентарь (см. документацию `docs/ingress/metallb.md` тега).
