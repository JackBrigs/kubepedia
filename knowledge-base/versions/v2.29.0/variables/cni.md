...
git_commit: 9991412
source_type: code
source_path: versions/v2.29.0/variables/cni.yaml
...
# Переменные CNI в Kubespray v2.29.0

Обзорная заметка по переменным сетевой подсистемы (CNI) для тега `v2.29.0` (коммит `9991412`).
**Источник истины — YAML-справочник** [[versions/v2.29.0/variables/cni|cni.yaml]] (131 переменная); эта заметка — его человекочитаемое изложение.
Срез версии: [[versions/v2.29.0/README|Срез v2.29.0]].
...
## Как разворачивается Cilium в v2.29.0
...
| `cilium_hubble_envoy_image_*` | cilium-envoy `v1.34.7-...` |
...
В `roles/network_plugin/` тега v2.29.0 существуют, но по ограничению проекта **не проиндексированы**: `calico`, `calico_defaults`, `flannel`, `kube-ovn`, `kube-router`, `macvlan`, `multus`, `custom_cni`, `ovn4nfv`.
