...
# Компоненты и их версии по умолчанию в Kubespray v2.29.0

Часть среза [[versions/v2.29.0/README|Срез v2.29.0]]. Источник истины — парный YAML-справочник [[versions/v2.29.0/components|components.yaml]] (там же дословные Jinja-выражения и source_path каждого компонента).

## Сводная таблица

| Компонент | Версия по умолчанию | Переменная |
|---|---|---|
| Kubernetes | 1.33.5 | `kube_version` |
| kubeadm / kubectl / kubelet | 1.33.5 (= `kube_version`) | `kube_version` |
| etcd | 3.5.23 | `etcd_version` |
| pause (pod infra) | 3.10 | `pod_infra_version` |
| containerd | 2.1.4 | `containerd_version` |
| runc | 1.3.2 | `runc_version` |
| crictl | 1.33.0 | `crictl_version` |
| nerdctl | 2.1.6 | `nerdctl_version` |
| CRI-O | 1.33.5 | `crio_version` |
| cri-dockerd | 0.3.20 | `cri_dockerd_version` |
...
| Cilium | 1.18.2 | `cilium_version` |
| cilium-cli | 0.18.7 | `cilium_cli_version` |
...
| Argo CD | 2.14.20 | `argocd_version` |
...
1. `kube_version` = первый ключ `kubelet_checksums['amd64']` → **1.33.5** (`roles/kubespray_defaults/defaults/main/main.yml`). Минимально допустимая версия `kube_version_min_required` = последний ключ → **1.31.0**.
...
3. Компоненты, привязанные к версии Kubernetes, выбираются из словарей `*_supported_versions` по ключу `kube_major_version`:
   - `etcd_version` = `etcd_supported_versions['1.33']` = «первый ключ `etcd_binary_checksums['amd64']` < 3.6» → **3.5.23**;
   - `coredns_version` = `coredns_supported_versions['1.33']` → **1.12.0**;
   - `pod_infra_version` = `pod_infra_supported_versions['1.33']` → **3.10**.
4. `crictl_version` и `crio_version` берут первый ключ своего словаря сумм, **строго меньший** `kube_major_next_version` (1.34): самая новая версия, совместимая с текущим Kubernetes → crictl **1.33.0**, CRI-O **1.33.5**.
5. Литералами заданы: `cilium_version` (1.18.2), `nodelocaldns_version`, версии addons (metrics-server, ingress-nginx, cert-manager, MetalLB и др.).

Механизм загрузки этих компонентов описан в [[versions/v2.29.0/variables/download|заметке о механизме download]].
