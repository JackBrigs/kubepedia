...
# Установка и базовая эксплуатация Kubespray v2.29.0

Дайджест по установке и первичной эксплуатации кластера, собранный строго из каталога `docs/` тега `v2.29.0` (commit `9991412`). Дословные команды, имена переменных и номера портов сохранены как в оригинале.

Связанные срезы базы: [[versions/v2.29.0/ansible-tags|Ansible-теги]], [[versions/v2.29.0/variables/k8s-cluster|Переменные ядра]], [[versions/v2.29.0/inventory/k8s-cluster|Inventory ядра]].
...
Примечание: в примере из документации фигурирует тег образа `v2.29.0` (пример приведён в docs тега v2.29.0 без изменений).

### 1.4. Kubespray как Ansible collection

Kubespray можно установить как Ansible-коллекцию (`docs/ansible/ansible_collection.md`):

1. подготовить инвентарь с нужными группами хостов и group_vars;
...
**Важно:** extra vars стоит применять только для переопределения внутренних переменных Kubespray (`roles/vars/`), которые не являются частью публичного интерфейса и могут меняться без предупреждения. Подробнее о переменных — [[versions/v2.29.0/variables/k8s-cluster|Переменные ядра]] (источник `docs/ansible/vars.md`).
...
Kubespray поддерживает выборочный запуск через `--tags` / `--skip-tags`. Полный справочник тегов и их поведения — [[versions/v2.29.0/ansible-tags|Ansible-теги]]. Примеры из `docs/ansible/ansible.md`:
...
Примечание: поддержка **Mitogen** объявлена устаревшей (deprecated).
...
Источник: `docs/getting_started/setting-up-your-first-cluster.md` — прикладной пошаговый гайд по развёртыванию первого кластера на Google Cloud Platform (3 controller + 3 worker на Ubuntu 24.04). Ключевые шаги:
...
Примечание: команды туториала используют `git checkout release-2.17` и образы с K8s v1.17.9 — это исторический пример из документации, а не версии, поставляемые тегом v2.29.0. Актуальные версии компонентов — см. [[versions/v2.29.0/components|Компоненты v2.29.0]].

---

## Источники

Все файлы — из каталога `docs/` тега `v2.29.0` (commit `9991412`):
...
Назад: [[versions/v2.29.0/README|Срез v2.29.0]]
