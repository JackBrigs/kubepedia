...
git_commit: a20891a
source_type: code
source_path: versions/v2.28.1/variables/etcd.yaml
...
# Переменные etcd — Kubespray v2.28.1

Человекочитаемая заметка к машиночитаемому справочнику `etcd.yaml` (65 переменных).
**Источник истины — YAML-справочник** `versions/v2.28.1/variables/etcd.yaml`; эта заметка — его изложение.
Назад к срезу: [[versions/v2.28.1/README|Срез v2.28.1]].

Источники в коде тега `v2.28.1` (commit `a20891a`):

- `roles/etcd/defaults/main.yml` — в v2.28.1 значения по умолчанию роли `etcd` лежат **прямо в её каталоге `defaults/`** (отдельной роли `roles/etcd_defaults`, как в v2.29.1, здесь ещё **нет**). Файл содержит 33 активных top-level ключа и 7 закомментированных;
...
## Типы развёртывания etcd в v2.28.1
...
> Важно: переменная `etcd_kubeadm_enabled` в коде v2.28.1 **удалена** — её
...
|---|---|---|
| `etcd_version` | `{{ etcd_supported_versions[kube_major_version] }}` | В v2.28.1 словарь `etcd_supported_versions` задаёт для `1.30`/`1.31`/`1.32` не литерал `3.5.16` (как в v2.28.0), а выражение `(etcd_binary_checksums['amd64'].keys() \| select('version', '3.6', '<'))[0]` — наибольшую версию etcd < 3.6 из checksums. При K8s по умолчанию 1.32.8 фактически **3.5.22** |
...
> В v2.28.1 переменной `etcd_log_level` в defaults роли etcd **нет** (в v2.29.1 она появляется).
