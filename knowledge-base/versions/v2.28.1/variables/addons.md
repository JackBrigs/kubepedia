...
git_commit: a20891a
source_type: code
source_path: versions/v2.28.1/variables/addons.yaml
...
# Переменные аддонов (kubernetes-apps) в Kubespray v2.28.1

Справочник переменных дополнительных компонентов, разворачиваемых ролями `roles/kubernetes-apps/*` на теге `v2.28.1` (коммит `a20891a`).

**Источник истины** — машиночитаемый справочник [[versions/v2.28.1/variables/addons|addons.yaml]] (367 переменных). Эта заметка — человекочитаемое изложение.

Назад к срезу: [[versions/v2.28.1/README|Срез v2.28.1]]

## Обзор

В v2.28.1 через `kubernetes-apps` доступны следующие аддоны:
...
- **MetalLB**;
- **Argo CD**;
- **CRD Gateway API** (роль `gateway_api` — в v2.28.1 расположена в `roles/kubernetes-apps/gateway_api/`, в v2.29.1 перенесена в `common_crds/gateway_api`);
...
## Отличия v2.28.1 от v2.29.1 (структурные)

- **Gateway API**: в v2.28.1 роль расположена в `roles/kubernetes-apps/gateway_api/`; в v2.29.1 перенесена в `roles/kubernetes-apps/common_crds/gateway_api/`.
- **Нет Prometheus Operator CRD**: роль `common_crds/prometheus_operator_crds` и флаг `prometheus_operator_crds_enabled` в v2.28.1 отсутствуют (появились в v2.29.1).
...
- **argocd**: в v2.28.1 `argocd_install_url` удалён из defaults роли и перенесён в `roles/kubespray_defaults/defaults/main/download.yml` (вместе с новой `argocd_install_checksum`); установка ArgoCD идёт через элемент `downloads.argocd_install` по контрольной сумме. В defaults роли остаётся `argocd_version: 2.14.5` как fallback.
- **container_engine_accelerator/nvidia_gpu**: в v2.28.1 у роли есть `defaults/main.yml` (13 ключей); в справочник не включён (как и в v2.29.1).
...
Отдельного ключа `coredns_replicas` в defaults роли `ansible` в v2.28.1 нет (число реплик при выключенном автоскейлере задаётся в другом месте); сам флаг `enable_nodelocaldns` относится к справочнику k8s-cluster (kubespray_defaults).
...
Пулы адресов (IPAddressPool, L2/BGP-advertisement) в defaults роли v2.28.1 не задаются — настраиваются пользователем через инвентарь.
...
| Переменная | Значение по умолчанию | Назначение |
|---|---|---|
| `argocd_version` | `2.14.5` | версия Argo CD (в v2.28.1 фактически резолвится в `download.yml` из `argocd_install_checksums.no_arch`; в defaults роли — fallback) |
| `argocd_namespace` | `argocd` | namespace |
| `argocd_install_url` | в `download.yml`: `https://raw.githubusercontent.com/argoproj/argo-cd/v{{ argocd_version }}/manifests/install.yaml` | источник манифеста (перенесён из роли; см. download.yaml) |
| `argocd_install_checksum` | в `download.yml`: `{{ argocd_install_checksums.no_arch[argocd_version] }}` | контрольная сумма манифеста (новая в v2.28.1) |
...
Роль в v2.28.1 — `roles/kubernetes-apps/gateway_api/` (не `common_crds/`).
...
- **Суммарное число ключей верхнего уровня** (`grep -cE '^[a-z_][a-z0-9_]*:'` по каждому файлу): **363** (в v2.28.1 из `roles/kubernetes-apps/argocd/defaults/main.yml` удалён `argocd_install_url` — было 364 в v2.28.0).
...
- **Извлечено из defaults ролей kubernetes-apps**: 363 − 13 = **350**.
...
- **Итого переменных в `addons.yaml`**: 350 + 17 = **367**.
- Примечание v2.28.1: `argocd_install_url` больше не в defaults роли argocd (перенесён в `download.yml`); вместе с новой `argocd_install_checksum` он описан в справочнике [[versions/v2.28.1/variables/download|download.yaml]].
