...
git_commit: a20891a
source_type: code
source_path: versions/v2.28.1/variables/container-runtime.yaml
...
# Container runtime в Kubespray v2.28.1

Справочник переменных container runtime для тега `v2.28.1` (commit `a20891a`, Kubernetes 1.32.8).
**Источник истины** — машинно-читаемый файл [[versions/v2.28.1/variables/container-runtime|container-runtime.yaml]] (84 переменные); эта заметка — его человекочитаемое изложение.
...
|---|---|
| `containerd_version` | первый ключ `containerd_archive_checksums['amd64']` → **2.0.6** в этом теге |
...
> **Отличие от v2.29.1:** в v2.28.1 **нет** переменной `containerd_static_binary` и нет префикса `static-` в `containerd_download_url` — URL всегда собирается как `containerd-{{ containerd_version }}-linux-{{ image_arch }}.tar.gz`.
...
> **Отличие от v2.29.1:** в v2.28.1 **нет** переменной `containerd_extra_runtime_args` (доп. опции CRI runtime-плагина `[plugins."io.containerd.cri.v1.runtime"]`). Доступен только `containerd_extra_args` (дословная вставка на верхний уровень config.toml).
...
- `containerd_registry_auth` — `[]`; список registry/username/password. Внимание: в теге используется только легаси-шаблоном `config-v1.toml.j2`, то есть при дефолтном containerd 2.0.6 не применяется (в YAML помечено `reliability: unconfirmed`).
...
> **Отличие от v2.29.1:** `crictl_version` в v2.28.1 берётся из явного словаря `crictl_supported_versions` в `download.yml` (для `'1.32'` → `1.32.0`), а не выражением `select('version', kube_major_next_version, '<')` по `crictl_checksums`.

## Непроиндексированные рантаймы

В теге v2.28.1 в `roles/container-engine/` существуют, но по переменным в этом справочнике **не разобраны**:
