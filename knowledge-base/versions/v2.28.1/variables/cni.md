...
git_commit: a20891a
source_type: code
source_path: versions/v2.28.1/variables/cni.yaml
...
# Переменные CNI в Kubespray v2.28.1

Обзорная заметка по переменным сетевой подсистемы (CNI) для тега `v2.28.1` (коммит `a20891a`, Kubernetes 1.32.8).
**Источник истины — YAML-справочник** [[versions/v2.28.1/variables/cni|cni.yaml]] (129 переменных); эта заметка — его человекочитаемое изложение.
Срез версии: [[versions/v2.28.1/README|Срез v2.28.1]].
...
## Как разворачивается Cilium в v2.28.1
...
Важно (отличие от более поздних тегов): в `apply.yml` команда — это **только** `cilium install` (без ветки `upgrade`, без второго `-f cilium-extra-values.yaml` и без дополнительных флагов). В `defaults/main.yml` этого тега **нет** переменных `cilium_install_extra_flags`, `cilium_extra_values` (они появляются позже, в v2.29.1). Произвольные helm-values и ресурсные лимиты в этом теге можно задать только правкой шаблона `values.yaml.j2` — отдельного механизма override нет.

Отличие v2.28.1 от v2.28.0 (форвард-патч): в v2.28.1 в роль Cilium добавлены переменные `cilium_remove_old_resources` (по умолчанию `false`; включает миграционную задачу `tasks/remove_old_resources.yml`, которая в этом теге уже присутствует; в комментарии кода помечена как временная — «remove migrate after 2.29 released») и `cilium_hubble_peer_service_cluster_domain` (по умолчанию `{{ dns_domain }}`; переопределяет DNS-суффикс для резолвинга peer-сервиса Hubble-Relay; продублирована закомментированной в `inventory/sample/group_vars/k8s_cluster/k8s-net-cilium.yml`). Также поднята версия Cilium: `cilium_version` 1.17.3 → 1.17.7.
...
Отдельно отметим специфику именно v2.28.1: в шаблоне `values.yaml.j2` этого тега **не рендерятся** `cilium_policy_audit_mode` (нет ключа `policyAuditMode`), `cilium_config_extra_vars` (нет `extraConfig`) и `cilium_enable_bgp_control_plane` (нет `bgpControlPlane.enabled`) — поэтому они помечены `unconfirmed`, хотя в более поздних версиях могут быть задействованы. BGP при этом работоспособен: управляется наличием непустых списков CRD (`cilium_bgp_*`), а не флагом control plane.
...
| `cilium_cli_version` | `{{ (ciliumcli_binary_checksums['amd64'] \| dict2items)[0].key }}` (в теге `0.18.6`) |
...
В `roles/network_plugin/` тега v2.28.1 существуют, но по ограничению проекта **не проиндексированы**: `calico`, `calico_defaults`, `flannel`, `weave`, `kube-ovn`, `kube-router`, `macvlan`, `multus`, `custom_cni`, `ovn4nfv`.
