...
# Расхождения sample-inventory ↔ roles/*/defaults (v2.28.1)
...
Проверено 608 переменных sample из 6 срезов (`k8s-cluster`, `kube_control_plane`, `addons`, `all`, `cloud-providers`, `cni-cilium`); реально заданы (не закомментированы) — 97. Закомментированные примеры в сравнении действующих значений не участвуют. Примечание v2.28.1: в `k8s-net-cilium.yml` добавлена закомментированная `cilium_hubble_peer_service_cluster_domain` (пример `{{ dns_domain }}` совпадает с default роли — расхождения нет).
...
- **Характер:** значения совпадают — расхождения нет. Это отличие от v2.29.1, где default вычислялся по CI-переменной окружения (`lookup('env', 'CI_PROJECT_URL') == ...`) и давал расхождение формы. В v2.28.1 default задан прямым `false`.
...
Связанные справочники: [[versions/v2.28.1/inventory/k8s-cluster|inventory/k8s-cluster]] · [[versions/v2.28.1/inventory/kube_control_plane|inventory/kube_control_plane]] · [[versions/v2.28.1/inventory/all|inventory/all]] · [[versions/v2.28.1/inventory/cni-cilium|inventory/cni-cilium]] · [[versions/v2.28.1/inventory/cloud-providers|inventory/cloud-providers]] · [[versions/v2.28.1/inventory/addons|inventory/addons]]
