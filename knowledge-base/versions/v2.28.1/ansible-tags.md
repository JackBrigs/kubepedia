...
git_commit: a20891a
source_type: code
source_path: versions/v2.28.1/ansible-tags.yaml
...
# Ansible-теги запуска Kubespray v2.28.1
...
- **Всего тегов в v2.28.1:** 125 (извлечены из `playbooks/*.yml` и `roles/*/tasks/**`, полнота сверена программно).
...
| `cni` | cluster.yml, upgrade-cluster.yml | safe | ВНИМАНИЕ: по коду тега (приоритет у кода над docs/ansible/ansible.md, где тег описан как «CNI plugins for Network Plugins») тег cni в v2.28.1 навешан… |
...
Во всех случаях приоритет отдан коду тега v2.28.1 (раздел 5.1 CLAUDE.md); расхождения зафиксированы в поле `notes` соответствующих записей YAML.
...
- **`weave` есть только в v2.28.1.** CNI-плагин Weave Net (тег `weave`) удалён в v2.29.1.
