...
# CNI и DNS-стек в v2.28.1

Дайджест документации `docs/` тега v2.28.1. В проекте детально индексируется только CNI **Cilium**; остальные плагины перечислены справочно. Также разобран DNS-стек (CoreDNS / nodelocaldns) и утилита проверки сети netchecker.

> Отличия от v2.29.1: (1) в `docs/CNI/cilium.md` тега v2.28.1 пример версии — `cilium_version: "1.17.7"` (в v2.29.1 — `"1.18.4"`); (2) в v2.28.1 ещё присутствует CNI **Weave** (`docs/CNI/weave.md`), удалённый в v2.29.1. Файлы `cni.md`, `dns-stack.md`, `netcheck.md` идентичны v2.29.1.

Связанные заметки: [[versions/v2.28.1/variables/cni|Переменные CNI]] · [[versions/v2.28.1/ansible-tags|Ansible-теги]] · [[versions/v2.28.1/README|Срез v2.28.1]].
...
- Версия Cilium — `cilium_version` (пример в доке тега v2.28.1: `"1.17.7"`).
...
> Примечание: режимы маршрутизации (tunnel / native routing) и подробные значения по умолчанию в тексте `docs/CNI/cilium.md` тега v2.28.1 отдельными разделами не описаны — конкретные дефолты и режим датаплейна см. в [[versions/v2.28.1/variables/cni|переменных CNI]] (роль `roles/network_plugin/cilium/defaults`).
...
- Weave — `docs/CNI/weave.md` — не проиндексирован детально (присутствует в v2.28.1; удалён в v2.29.1).
...
- `docs/CNI/cilium.md` (v2.28.1)
- `docs/CNI/cni.md` (v2.28.1)
- `docs/advanced/dns-stack.md` (v2.28.1)
- `docs/advanced/netcheck.md` (v2.28.1)
...
- Репозиторий тега: https://github.com/kubernetes-sigs/kubespray/tree/v2.28.1
