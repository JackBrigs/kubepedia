...
# Обновление кластера в v2.28.1

Дайджест построен строго по документации тега `v2.28.1` (commit `a20891a`): `docs/operations/upgrades.md` и `docs/upgrades/migrate_docker2containerd.md`. Это выжимка, а не дословный перевод; имена переменных, тегов и команд приведены точно.

> Отличия `docs/operations/upgrades.md` от v2.29.1: в v2.28.1 в списке переменных версий компонентов присутствует `weave_version`, а в порядке обновления упоминается «network_plugin (such as Calico or Weave)». В v2.29.1 упоминания Weave удалены. Файл `migrate_docker2containerd.md` идентичен v2.29.1.
...
- `calico_version`
- `calico_cni_version`
- `weave_version` (в v2.28.1 ещё присутствует; удалён в v2.29.1)
- `flannel_version`
...
- Это ровно тот принцип, на котором построена база знаний: последовательность `v2.28.1 → v2.29.0 → v2.29.1 → ...` без пропусков.
...
Подробности по тегам запуска: [[versions/v2.28.1/ansible-tags|Ansible-теги запуска v2.28.1]].
...
Связанные переменные: [[versions/v2.28.1/variables/etcd|Переменные etcd v2.28.1]].
...
- `docs/operations/upgrades.md` (тег v2.28.1)
- `docs/upgrades/migrate_docker2containerd.md` (тег v2.28.1)
