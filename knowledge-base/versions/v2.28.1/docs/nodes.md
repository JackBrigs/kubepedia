...
# Управление узлами и восстановление control plane в v2.28.1

Дайджест построен строго по документации тега `v2.28.1` (commit `a20891a`): `docs/operations/nodes.md` и `docs/operations/recover-control-plane.md`. Выжимка, не дословный перевод; имена плейбуков, переменных и команд приведены точно.

> Отличие от v2.29.1: в v2.28.1 раздел про удаление первого control plane состоит из двух частей — вверху файла раздел «Limitation: Removal of first kube_control_plane and etcd-master» (примеры инвентаря в формате **YAML**) и ниже отдельный раздел «Replacing a first control plane node» (переупорядочивание инвентаря в формате **INI**). В v2.29.1 эти части были объединены в один раздел с YAML-примерами. Операционное содержание идентично.
...
Связанные срезы: [[versions/v2.28.1/ansible-tags|Ansible-теги запуска v2.28.1]] (reset, pre-remove, post-remove), [[versions/v2.28.1/variables/etcd|Переменные etcd v2.28.1]].

## Источники

- `docs/operations/nodes.md` (тег v2.28.1)
- `docs/operations/recover-control-plane.md` (тег v2.28.1)
