...
# Установка и базовая эксплуатация Kubespray v2.28.1

Дайджест по установке и первичной эксплуатации кластера, собранный строго из каталога `docs/` тега `v2.28.1` (commit `a20891a`). Дословные команды, имена переменных и номера портов сохранены как в оригинале.
...
Связанные срезы базы: [[versions/v2.28.1/ansible-tags|Ansible-теги]], [[versions/v2.28.1/variables/k8s-cluster|Переменные ядра]], [[versions/v2.28.1/inventory/k8s-cluster|Inventory ядра]].
...
Таблица совместимости Ansible и Python (из `docs/ansible/ansible.md` тега v2.28.1):
...
Примечание: в примере из документации тега v2.28.1 фигурирует тег образа `v2.27.0` (в v2.29.1 пример обновлён до `v2.29.0`).

### 1.4. Kubespray как Ansible collection

Kubespray можно установить как Ansible-коллекцию (`docs/ansible/ansible_collection.md`). В тексте тега v2.28.1 присутствует раздел **Requirements** (удалён в v2.29.1):
...
Порядок использования (шаги 1–4 в v2.28.1):
...
**Важно:** extra vars стоит применять только для переопределения внутренних переменных Kubespray (`roles/vars/`), которые не являются частью публичного интерфейса и могут меняться без предупреждения. Подробнее о переменных — [[versions/v2.28.1/variables/k8s-cluster|Переменные ядра]] (источник `docs/ansible/vars.md`).
...
Kubespray поддерживает выборочный запуск через `--tags` / `--skip-tags`. Полный справочник тегов и их поведения — [[versions/v2.28.1/ansible-tags|Ansible-теги]]. Примеры из `docs/ansible/ansible.md`:
...
Примечания по v2.28.1: в списке Ansible-тегов ещё присутствуют тег `master (DEPRECATED)` (в v2.29.1 удалён, вместо него `control-plane`) и тег `weave` (удалён в v2.29.1 вместе с CNI Weave); есть примечание, что список тегов можно сгенерировать командой `bash scripts/gen_tags.sh`. Поддержка **Mitogen** объявлена устаревшей (deprecated).
...
1. подготовка инфраструктуры GCP (VPC `kubernetes-the-kubespray-way`, subnet `10.240.0.0/24`, firewall для internal и external доступа: `tcp:80,tcp:6443,tcp:443,tcp:22,icmp`); важно разрешить vxlan для работы Calico — в примере тега v2.28.1 internal-правило задаётся как `--allow tcp,udp,icmp,vxlan` (в v2.29.1 — `tcp,udp,icmp`);
...
Примечание: команды туториала используют `git checkout release-2.17` и образы с K8s v1.17.9 — это исторический пример из документации, а не версии, поставляемые тегом v2.28.1. Актуальные версии компонентов — см. [[versions/v2.28.1/components|Компоненты v2.28.1]].

---

## Источники

Все файлы — из каталога `docs/` тега `v2.28.1` (commit `a20891a`):
...
Назад: [[versions/v2.28.1/README|Срез v2.28.1]]
