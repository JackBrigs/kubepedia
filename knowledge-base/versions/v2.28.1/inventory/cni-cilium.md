...
# Sample-inventory: настройки Cilium (v2.28.1)

Разбор файла `inventory/sample/group_vars/k8s_cluster/k8s-net-cilium.yml` тега
**v2.28.1** (commit `a20891a`). Из сетевых плагинов в проекте индексируется
**только Cilium**.

Источник истины по машиночитаемым данным — [[versions/v2.28.1/inventory/cni-cilium|cni-cilium.yaml]].
Значения по умолчанию самой роли — в [[versions/v2.28.1/variables/cni|variables/cni.yaml]].
Родительский срез: [[versions/v2.28.1/README|Срез v2.28.1]].
...
| `cilium_hubble_peer_service_cluster_domain` | `{{ dns_domain }}` | DNS-суффикс резолвинга peer-сервиса Hubble-Relay (новая в v2.28.1) |
...
[[versions/v2.28.1/variables/cni|variables/cni.yaml]].
...
Из сетевых плагинов v2.28.1 индексируется только Cilium. Следующие sample-файлы
...
- `inventory/sample/group_vars/k8s_cluster/k8s-net-weave.yml` (присутствует в v2.28.1, удалён в v2.29.1)
- `inventory/sample/group_vars/k8s_cluster/k8s-net-custom-cni.yml`

> [!note] Отличия набора переменных Cilium от v2.29.1
> В v2.28.1 sample-файл уже содержит (закомментированной) `cilium_hubble_peer_service_cluster_domain`
> (добавлена в v2.28.1 относительно v2.28.0), но **нет** `cilium_extra_values` (helm passthrough,
> добавлена в v2.29.1). Итог: 82 переменные против 83 в v2.29.1.
