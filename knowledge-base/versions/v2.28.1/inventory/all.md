...
# Общие настройки инвентаря (group_vars/all) — v2.28.1
...
> В v2.29.1 этот файл **удалён** — соответствующие параметры перенесены в defaults роли `etcd`. В v2.28.1 файл ещё присутствует в sample-inventory.
...
`roles/kubespray_defaults/defaults/main/download.yml` тега v2.28.1 задан прямым `false`
и совпадает с sample (в отличие от v2.29.1, где default вычисляется по CI-переменной
окружения — там это давало расхождение формы). Подробнее см. [[versions/v2.28.1/discrepancies|discrepancies.md]] среза.
