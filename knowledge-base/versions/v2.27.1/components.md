...
# Компоненты и их версии по умолчанию в Kubespray v2.27.1

Часть среза [[versions/v2.27.1/README|Срез v2.27.1]]. Источник истины — парный YAML-справочник [[versions/v2.27.1/components|components.yaml]] (там же дословные выражения и source_path каждого компонента).

## Сводная таблица

| Компонент | Версия по умолчанию | Переменная |
|---|---|---|
| Kubernetes | 1.31.9 | `kube_version` |
| kubeadm / kubectl / kubelet | 1.31.9 (= `kube_version`) | `kube_version` |
| etcd | 3.5.21 | `etcd_version` |
| pause (pod infra) | 3.10 | `pod_infra_version` |
| containerd | 1.7.27 | `containerd_version` |
...
| CRI-O | 1.31.6 | `crio_version` |
| cri-dockerd | 0.3.16 | `cri_dockerd_version` |
...
| cilium-cli | 0.16.24 | `cilium_cli_version` |
| Calico | 3.29.4 | `calico_version` |
...
В отличие от v2.29.x, в v2.27.1 CNI-плагины (Calico, Flannel, Weave, kube-ovn, kube-router, Multus) задекларированы прямо в `roles/kubespray-defaults/defaults/main/download.yml` литералами и включены в справочник. CNI по умолчанию у проекта — Cilium.

## Как версии вычисляются

В v2.27.1 механизм проще, чем в v2.29.x: большинство версий заданы **прямыми литералами** в `roles/kubespray-defaults/defaults/main/download.yml`, например:
...
1. `kube_version` = литерал **v1.31.9** (`roles/kubespray-defaults/defaults/main/main.yml`). Минимально допустимая `kube_version_min_required` = **v1.29.0**.
...
4. `coredns_version` выбирается тернарным условием по `kube_version`: `v1.11.3`, если `kube_version >= v1.30.5`, иначе `v1.11.1`; для v1.31.9 → **v1.11.3**.
5. Литералами заданы почти все остальные компоненты, включая CNI (Cilium 1.15.9, Calico 3.29.4 и др.), containerd 1.7.27, runc v1.2.6, Helm v3.16.4, addon-ы (metrics-server, ingress-nginx, cert-manager, MetalLB и т.д.).
