...
# Установка и базовая эксплуатация Kubespray v2.27.1

Дайджест по установке и первичной эксплуатации кластера, собранный строго из каталога `docs/` тега `v2.27.1` (commit `45140b5`). Дословные команды, имена переменных и номера портов сохранены как в оригинале.

Связанные срезы базы: [[versions/v2.27.1/ansible-tags|Ansible-теги]], [[versions/v2.27.1/variables/k8s-cluster|Переменные ядра]], [[versions/v2.27.1/inventory/k8s-cluster|Inventory ядра]].
...
Таблица совместимости Ansible и Python (из `docs/ansible/ansible.md`):

| Версия Ansible | Версия Python |
|----------------|---------------|
| >= 2.16.4      | 3.10-3.12     |

Если pip выдаёт ошибку `No matching distribution found for ansible==7.6.0` — версия Python несовместима с поддерживаемой Kubespray версией Ansible (если максимум по pip — `6.7.0`, значит используется Python 3.8 или ниже, а нужен минимум Python 3.9).
...
Примечание: в примере из документации `docs/ansible/ansible.md` тега v2.27.1 фигурирует тег образа `v2.26.0` (пример приведён без изменений). Также в разделе Troubleshooting документа указано, что Kubespray поставляет собственные модули, путь к которым задаётся через `export ANSIBLE_LIBRAY=<kubespray_dir>/library`.

### 1.4. Kubespray как Ansible collection

Kubespray можно установить как Ansible-коллекцию (`docs/ansible/ansible_collection.md`):

1. подготовить инвентарь с нужными группами хостов; `group_vars` должны соответствовать именам переменных из `inventory/local/group_vars`;
2. добавить Kubespray в `requirements.yml` (тип `git`, поле `version` — нужный тег или ветка);
3. установить: `ansible-galaxy install -r requirements.yml`;
4. импортировать плейбук в свой: `ansible.builtin.import_playbook: kubernetes_sigs.kubespray.cluster`;
5. запустить: `ansible-playbook -i INVENTORY --become --become-user=root PLAYBOOK`.

### 1.5. Kubespray как git submodule

`docs/operations/integration.md` описывает включение Kubespray в собственный ansible-репозиторий через git submodule: форк репозитория, добавление форка как submodule (`git submodule add`), настройка upstream, правка `ansible.cfg` (`library`, `roles_path` с путями к `3d/kubespray/`), маппинг существующих групп инвентаря на группы Kubespray (`kube_node:children`, `etcd:children`, `kube_control_plane:children`) и включение через `import_playbook: 3d/kubespray/cluster.yml`.

### 1.6. Требования к SSH
...
**Важно:** extra vars стоит применять только для переопределения внутренних переменных Kubespray (`roles/vars/`), которые не являются частью публичного интерфейса и могут меняться без предупреждения. Подробнее о переменных — [[versions/v2.27.1/variables/k8s-cluster|Переменные ядра]] (источник `docs/ansible/vars.md`).
...
Kubespray поддерживает выборочный запуск через `--tags` / `--skip-tags`. Полный справочник тегов и их поведения — [[versions/v2.27.1/ansible-tags|Ansible-теги]]. Примеры из `docs/ansible/ansible.md`:
...
Предупреждение из документации: использовать `--tags`/`--skip-tags` осознанно, только при полном понимании последствий. Список всех тегов можно сгенерировать командой `bash scripts/gen_tags.sh`.

Примечание: поддержка **Mitogen** объявлена устаревшей (deprecated) — см. [[versions/v2.27.1/docs/proxy|Прокси и Mitogen]].
...
В документе приведена таблица автоматического выбора endpoint'ов доступа к API (Local LB по умолчанию, Local LB + custom bind IP, external LB и т. д.).
...
Поддерживается kubernetes-dashboard v2.0.x. По умолчанию разворачивается в namespace `kube-system` (переопределяется `dashboard_namespace`), работает только по https. Proxy URL: `http://localhost:8001/api/v1/namespaces/kube-system/services/https:kubernetes-dashboard:/proxy/#/login`. Рекомендуется доступ через Ingress.
...
Источник: `docs/getting_started/setting-up-your-first-cluster.md` — прикладной пошаговый гайд по развёртыванию первого кластера на Google Cloud Platform (3 controller + 3 worker на Ubuntu Server 18.04). Ключевые шаги:

1. подготовка инфраструктуры GCP (VPC `kubernetes-the-kubespray-way`, subnet `10.240.0.0/24`, firewall для internal и external доступа: `tcp:80,tcp:6443,tcp:443,tcp:22,icmp`); важно разрешить vxlan (udp) для работы Calico;
2. создание 3 controller и 3 worker инстансов, настройка SSH-доступа;
...
Примечание: команды туториала используют `git checkout release-2.17` и образы с K8s v1.17.9 — это исторический пример из документации, а не версии, поставляемые тегом v2.27.1. Актуальные версии компонентов — см. [[versions/v2.27.1/components|Компоненты v2.27.1]].

---

## Источники

Все файлы — из каталога `docs/` тега `v2.27.1` (commit `45140b5`):
...
Назад: [[versions/v2.27.1/README|Срез v2.27.1]]
