...
# CNI и DNS-стек в v2.27.1

Дайджест документации `docs/` тега v2.27.1. В проекте детально индексируется только CNI **Cilium**; остальные плагины перечислены справочно. Также разобран DNS-стек (CoreDNS / nodelocaldns) и утилита проверки сети netchecker.

Связанные заметки: [[versions/v2.27.1/variables/cni|Переменные CNI]] · [[versions/v2.27.1/ansible-tags|Ansible-теги]] · [[versions/v2.27.1/README|Срез v2.27.1]].
...
- `cilium_loadbalancer_ip_pools` — список пулов LB IPAM с полями `name`, `cidrs`.
...
- Версия Cilium — `cilium_version` (пример в доке: `v1.12.1`).
...
> Примечание: режимы маршрутизации (tunnel / native routing) и подробные значения по умолчанию в тексте `docs/CNI/cilium.md` тега v2.27.1 отдельными разделами не описаны — конкретные дефолты и режим датаплейна см. в [[versions/v2.27.1/variables/cni|переменных CNI]] (роль `roles/network_plugin/cilium/defaults`).
...
- Weave — `docs/CNI/weave.md` — не проиндексирован детально.
...
- `searchdomains` — дополнительные search-домены (сверх кластерных `default.svc.{{ dns_domain }}`, `svc.{{ dns_domain }}`). Лимит систем: 6 имён и 256 символов суммарно. `resolvconf_mode: docker_dns` автоматически добавляет системные search-домены — это учитывается в лимитах.
...
- `docs/CNI/cilium.md` (v2.27.1)
- `docs/CNI/cni.md` (v2.27.1)
- `docs/advanced/dns-stack.md` (v2.27.1)
- `docs/advanced/netcheck.md` (v2.27.1)
- Прочие CNI (перечислены справочно, не проиндексированы): `docs/CNI/calico.md`, `docs/CNI/flannel.md`, `docs/CNI/kube-ovn.md`, `docs/CNI/kube-router.md`, `docs/CNI/macvlan.md`, `docs/CNI/multus.md`, `docs/CNI/weave.md`
- Репозиторий тега: https://github.com/kubernetes-sigs/kubespray/tree/v2.27.1
