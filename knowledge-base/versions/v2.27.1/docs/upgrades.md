...
# Обновление кластера в v2.27.1

Дайджест построен строго по документации тега `v2.27.1` (commit `45140b5`): `docs/operations/upgrades.md` и `docs/upgrades/migrate_docker2containerd.md`. Это выжимка, а не дословный перевод; имена переменных, тегов и команд приведены точно.
...
- Это ровно тот принцип, на котором построена база знаний: последовательность без пропусков.
...
4. При переходе через границы версий сверять свой инвентарь с sample-инвентарём: форматы переменных между версиями меняются. Пример из документа: между 2.8.5 и 2.9.0 поменялся формат переменных в `k8s_cluster.yml`; если не обновить копию инвентаря — **обновление упадёт**, и первый master останется нерабочим до починки и повторного запуска. Также в примере обновления с 2.6.0 до 2.10.0 фигурирует deprecation-предупреждение о переходе с non-kubeadm на kubeadm-режим (удалён в 2.9.0).
...
Затем повтор с новой версией (`-e kube_version=v1.19.7`).
...
- **Перед** использованием `--limit` обязательно прогнать `facts.yml` без limit, чтобы обновить кэш фактов по всем узлам:
...
## Обновление до v2.19 (историческое предостережение по etcd)
...
5. network_plugin (например, Calico или Weave)
...
Подробности по тегам запуска: [[versions/v2.27.1/ansible-tags|Ansible-теги запуска v2.27.1]].
...
Тестовое окружение из документа: Ubuntu 18.04 LTS, без cloud provider (baremetal/VM), Kubernetes 1.21.5, Kubespray 2.18.0.
...
- `docs/operations/upgrades.md` (тег v2.27.1)
- `docs/upgrades/migrate_docker2containerd.md` (тег v2.27.1)
