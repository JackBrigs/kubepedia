...
git_commit: 45140b5
source_type: code
source_path: versions/v2.27.1/variables/etcd.yaml
...
# Переменные etcd — Kubespray v2.27.1

Человекочитаемая заметка к машиночитаемому справочнику `etcd.yaml`.
**Источник истины — YAML-справочник** `versions/v2.27.1/variables/etcd.yaml`; эта заметка — его изложение.
...
## Типы развёртывания etcd в v2.27.1
...
|---|---|---|
| `etcd_version` | `{{ etcd_supported_versions[kube_major_version] }}` → **v3.5.21** | Для всех поддерживаемых версий K8s (1.29/1.30/1.31) — одна и та же v3.5.21. Значение **уже содержит префикс v** |
...
В v2.27.1 адреса строятся на переменных узла `ip` / `access_ip` / `fallback_ip`
