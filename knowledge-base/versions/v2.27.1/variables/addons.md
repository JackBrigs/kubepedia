...
git_commit: 45140b5
source_type: code
source_path: versions/v2.27.1/variables/addons.yaml
...
# Переменные аддонов (kubernetes-apps) в Kubespray v2.27.1

Справочник переменных дополнительных компонентов, разворачиваемых ролями `roles/kubernetes-apps/*` на теге `v2.27.1` (коммит `45140b5`).

**Источник истины** — машиночитаемый справочник [[versions/v2.27.1/variables/addons|addons.yaml]] (409 переменных: 390 из defaults ролей `kubernetes-apps` + 19 флагов-переключателей, определённых только в `roles/kubespray-defaults`). Эта заметка — человекочитаемое изложение.

Назад к срезу: [[versions/v2.27.1/README|Срез v2.27.1]]

## Обзор

В v2.27.1 через `kubernetes-apps` доступны следующие аддоны:
...
## Отличия структуры v2.27.1 от более новых версий
...
- флаги `deploy_coredns`, `enable_dns_autoscaler`, `enable_nodelocaldns` в v2.27.1 отсутствуют в defaults `kubernetes-apps/ansible` — включение DNS-стека управляется переменными k8s-cluster (`enable_dns_autoscaler`/`enable_nodelocaldns` — в `roles/kubespray-defaults`), здесь описаны только ресурсы/настройки подов.
...
Роль `roles/helm-apps` в v2.27.1 каталога `defaults/` **не имеет** — её параметры (`helm_update`, `helm_defaults`, `helm_repository_defaults`) лежат в `roles/helm-apps/vars/main.yml` и в справочник defaults не включены (в подсчёт полноты не входят).
